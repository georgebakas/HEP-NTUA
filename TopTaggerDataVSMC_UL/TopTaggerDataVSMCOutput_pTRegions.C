 ...
 for(int f=0; f<listOfFiles.size(); f++)
 {
  ...
  if (nJets >1)
  {
   ...
   //     jetBtagSub1_->push_back( (*jetBtagSub1)[(*partonMatchIdx)[indexMin]]);
    ...
    ptCutLeading[3] = ((*pt_)[0] > 1200);

    ptCutSubLeading[0] = ((*pt_)[1] > 400) && ((*pt_)[1] < 600);
    ptCutSubLeading[1] = ((*pt_)[1] > 600) && ((*pt_)[1] < 800);
   ...
   if(isMatched > 1)
    {
     ...
     ptCutLeading[0] = ((*pt_)[leadingPt] > 400) && ((*pt_)[leadingPt] < 600);
     ptCutLeading[1] = ((*pt_)[leadingPt] > 600) && ((*pt_)[leadingPt] < 800);
     ptCutLeading[2] = ((*pt_)[leadingPt] > 800) && ((*pt_)[leadingPt] < 1200);
     ptCutLeading[3] = ((*pt_)[leadingPt] > 1200);

     ptCutSubLeading[0] = ((*pt_)[subleadingPt] > 400) && ((*pt_)[subleadingPt] < 600);
     ptCutSubLeading[1] = ((*pt_)[subleadingPt] > 600) && ((*pt_)[subleadingPt] < 800);
  ...
  else //we are in QCD samples or Subdominant BKG or Data sample
  {
    ...
    ptCutLeading[0] = ((*jetPt)[0] > 400) && ((*jetPt)[0] < 600);
    ptCutLeading[1] = ((*jetPt)[0] > 600) && ((*jetPt)[0] < 800);
    ptCutLeading[2] = ((*jetPt)[0] > 800) && ((*jetPt)[0] < 1200);
    ptCutLeading[3] = ((*jetPt)[0] > 1200);

    ptCutSubLeading[0] = ((*jetPt)[1] > 400) && ((*jetPt)[1] < 600);
    ptCutSubLeading[1] = ((*jetPt)[1] > 600) && ((*jetPt)[1] < 800);
    ptCutSubLeading[2] = ((*jetPt)[1] > 800) && ((*jetPt)[1] < 1200);
    ptCutSubLeading[3] = ((*jetPt)[1] > 1200);
  ...
  if(selection ==0)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_Data_%s_100%s.root",year.Data(),year.Data(),loose.Data()), "RECREATE");
  else if(selection ==1)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_TT_Mtt-700toInf_100%s.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==2)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_QCD_HT300toInf_100%s.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==3)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_SubdominantBkgs_100%s.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==4)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_TT_NominalMC_100%s.root",year.Data(),loose.Data()), "RECREATE");
