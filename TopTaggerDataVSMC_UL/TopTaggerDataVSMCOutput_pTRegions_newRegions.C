...
void TopTaggerDataVSMCOutput_pTRegions(TString y="2016", int sel = 0, bool isLoose=false)
{
  year =y;
 ...
 TH1F *hCR_Leading[listOfFiles.size()][NPARTITIONS], *hCR_SubLeading[listOfFiles.size()][NPARTITIONS];
 TH1F *hSR_Leading[listOfFiles.size()][NPARTITIONS], *hSR_SubLeading[listOfFiles.size()][NPARTITIONS];
 ...
 for(int f=0; f<listOfFiles.size(); f++)
 {
  ...
  float xMin[NPARTITIONS] = {-1,-1,-1,-1};
  float xMax[NPARTITIONS] = {1,1,1,1};
  ...
  if (nJets >1)
  {
   ...
   //     jetBtagSub1_->push_back( (*jetBtagSub1)[(*partonMatchIdx)[indexMin]]);
    ...
    ptCutLeading[0] = ((*pt_)[0] > 300) && ((*pt_)[0] < 400);
    ptCutLeading[1] = ((*pt_)[0] > 400) && ((*pt_)[0] < 500);
    ptCutLeading[2] = ((*pt_)[0] > 500) && ((*pt_)[0] < 600);
    ptCutLeading[3] = ((*pt_)[0] > 600);

    ptCutSubLeading[0] = ((*pt_)[1] > 300) && ((*pt_)[1] < 400);
    ptCutSubLeading[1] = ((*pt_)[1] > 400) && ((*pt_)[1] < 500);
    ptCutSubLeading[2] = ((*pt_)[1] > 500) && ((*pt_)[1] < 600);
    ptCutSubLeading[3] = ((*pt_)[1] > 600);
   ...
   if(isMatched > 1)
    {
     ...
     ptCutLeading[0] = ((*pt_)[leadingPt] > 300) && ((*pt_)[leadingPt] < 400);
     ptCutLeading[1] = ((*pt_)[leadingPt] > 400) && ((*pt_)[leadingPt] < 500);
     ptCutLeading[2] = ((*pt_)[leadingPt] > 500) && ((*pt_)[leadingPt] < 600);
     ptCutLeading[3] = ((*pt_)[leadingPt] > 600);

     ptCutSubLeading[0] = ((*pt_)[subleadingPt] > 300) && ((*pt_)[subleadingPt] < 400);
     ptCutSubLeading[1] = ((*pt_)[subleadingPt] > 400) && ((*pt_)[subleadingPt] < 500);
     ptCutSubLeading[2] = ((*pt_)[subleadingPt] > 500) && ((*pt_)[subleadingPt] < 600);
     ptCutSubLeading[3] = ((*pt_)[subleadingPt] > 600);
  ...
  else //we are in QCD samples or Subdominant BKG or Data sample
  {
    ...
    ptCutLeading[0] = ((*jetPt)[0] > 300) && ((*jetPt)[0] < 400);
    ptCutLeading[1] = ((*jetPt)[0] > 400) && ((*jetPt)[0] < 500);
    ptCutLeading[2] = ((*jetPt)[0] > 500) && ((*jetPt)[0] < 600);
    ptCutLeading[3] = ((*jetPt)[0] > 600);

    ptCutSubLeading[0] = ((*jetPt)[1] > 300) && ((*jetPt)[1] < 400);
    ptCutSubLeading[1] = ((*jetPt)[1] > 400) && ((*jetPt)[1] < 500);
    ptCutSubLeading[2] = ((*jetPt)[1] > 500) && ((*jetPt)[1] < 600);
    ptCutSubLeading[3] = ((*jetPt)[1] > 600);
  ...
  TH1F *hCR_Leading_Clone[listOfFiles.size()][NPARTITIONS];
  TH1F *hCR_SubLeading_Clone[listOfFiles.size()][NPARTITIONS];
  TH1F *hSR_Leading_Clone[listOfFiles.size()][NPARTITIONS];
  TH1F *hSR_SubLeading_Clone[listOfFiles.size()][NPARTITIONS];
  ...
  if(selection ==0)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_Data_%s_100%s_newRegions.root",year.Data(),year.Data(),loose.Data()), "RECREATE");
  else if(selection ==1)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_TT_Mtt-700toInf_100%s_newRegions.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==2)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_QCD_HT300toInf_100%s_newRegions.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==3)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_SubdominantBkgs_100%s_newRegions.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==4)
    outFile = new TFile(TString::Format("%s/TopTaggerHisto_pTRegions_TT_NominalMC_100%s_newRegions.root",year.Data(),loose.Data()), "RECREATE");
