...
void CreateTreeForMVATraining_Matching(TString TREENAME, bool matching = true)
{
  ///eos/cms/store/user/ipapakri/ttbar/MC/Signal/2018/

  TFile *inf     = TFile::Open("/eos/cms/store/user/ipapakri/ttbar/MC/Bkg/2018/"+TREENAME+".root");
  TTree *trIN    = (TTree*)inf->Get("boosted/events");
  //When opening a file from eos
  //TString outFileName = formOutFileName(TREENAME+".root");
  TFile *outf    = TFile::Open("/eos/cms/store/user/gbakas/ttbar/topTagger/mc-2018/Bkg/"+TREENAME+"_Train.root","RECREATE");
