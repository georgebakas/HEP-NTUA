...
void perform_event_selection(observable_set & oset, call_generic & generic){
  ...
  */
  logger << LOG_DEBUG << temp_psp.str() << endl;
...
void perform_event_selection_phasespace(int i_a, observable_set & oset){
  ...
  static vector<fourvector> jet_axes;
  ...
  if (initialization == 1){
    ...
    if (osi_switch_qTcut == 3){
      ...
      */
      jet_axes.resize(2 + Njettiness); // not generic !!! there should be n_jet_born or so... !!!
      jet_axes[0] = fourvector(1., 0., 0., 1.);
      jet_axes[1] = fourvector(1., 0., 0., -1.);
    }
  ...
  if (osi_photon_recombination){photon_recombination(no_unrecombined_photon, i_a, oset);}
  ...
  */
  ...
  if (osi_frixione_isolation){
	...
	if (abs(osi_particle_event[0][i_a][i_p].rapidity) < osi_esi.pds[j_g].define_y && 
	    ...
	    osi_particle_event[0][i_a][i_p].pT >= osi_esi.pds[j_g].define_pT && 
	    osi_particle_event[0][i_a][i_p].ET >= osi_esi.pds[j_g].define_ET){
	  int temp_number_photon = osi_n_object[k_g][i_a];
	  ...
	  if (temp_number_photon == osi_n_object[k_g][i_a] && osi_photon_jet_algorithm){  //  Non-identified photons will enter the jet algorithm afterwards.
  ...
  if (osi_switch_qTcut == 3){
	...
	jet_axes[2 + i_j] = fourvector(1., temp_jet.x1() / temp_jet_R, temp_jet.x2() / temp_jet_R, temp_jet.x3() / temp_jet_R);
      }
      ...
      //      logger << LOG_INFO << "Njettiness = " << Njettiness << "   osi_ps_runtime_jet_algorithm[" << i_a << "].size() = " << osi_ps_runtime_jet_algorithm[i_a].size() << " --- " << temp.str() << endl;
      
      // calculate N-jettiness tau
      for (int i_p = 0; i_p < osi_ps_runtime_jet_algorithm[i_a].size(); i_p++){
	vector<double> delta_tau(2 + Njettiness);
	for (int i_j = 0; i_j < jet_axes.size(); i_j++){
	  //osi_particle_event[0][i_a][i_p]
	  delta_tau[i_j] = abs(jet_axes[i_j] * osi_particle_event[0][i_a][osi_ps_runtime_jet_algorithm[i_a][i_p]].momentum);
	  //	  delta_tau[i_j] = jet_axes[i_j] * osi_p_parton[i_a][osi_ps_runtime_jet_algorithm[i_a][i_p]];
	  //	  logger << LOG_INFO << "jet_axes[" << i_j << "] = " << jet_axes[i_j] << "   delta_tau[" << i_j << "] = " << delta_tau[i_j] << endl;
	  /*
	  if (delta_tau[i_j] < 0.){
	    logger << LOG_INFO << "jet_axes[" << i_j << "] * osi_particle_event[0][" << i_a << "][" << osi_ps_runtime_jet_algorithm[i_a][i_p] << "] is negative: delta_tau[" << i_j << "] = " << delta_tau[i_j] << endl;
	...
	*/
	...
	tau_jettiness += *min_element(delta_tau.begin(), delta_tau.end());
	//	tau_jettiness = tau_jettiness + *min_element(delta_tau.begin(), delta_tau.end());
	//	logger << LOG_INFO << "i_p = " << i_p << "   tau_jettiness = " << tau_jettiness << endl;
      }
      
      //      logger << LOG_INFO << "Njettiness = " << Njettiness << "   tau_jettiness = " << tau_jettiness << "   osi_min_qTcut = " << osi_min_qTcut << endl;
      if (tau_jettiness < osi_min_qTcut){osi_cut_ps[i_a] = -1; return;}
      else {osi_cut_ps[i_a] = GSL_MIN_INT(int((tau_jettiness - osi_min_qTcut) / osi_step_qTcut), osi_n_qTcut - 1);}
