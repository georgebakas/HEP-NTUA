...
void munich::integration_RA_QEW(){
  static Logger logger("munich::integration_RA_QEW");
  ...
  psi.MC_sum_channel_phasespace[0] = psi.MC_n_channel_phasespace[0]; // ??? not in QCD version
  ...
  calculate_ME2check_RA_QEW(psi, osi);
  ...
  while (osi.int_end == 0){
    ...
    for (int j = 0; j < psi.MC_phasespace.alpha.size(); j++){
      psi.MC_phasespace.g_channel[j] = 0.;
    }
