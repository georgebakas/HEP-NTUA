     ...
     .          +ca/4d0*(Mgs0(2,3)-Mgs0(3,3))*c3gs
      ...
      do i=1,4
        do j=i,4
           do ii=1,4
              do jj=1,4
                 Tgg4(i,j,ii,jj)=Tgg(i,j)*Tgg(ii,jj)
              enddo
           enddo
        enddo
      enddo
...
c      do i=1,4
c         Tgg4(i,i,i,i)=Tgg(i,i)*Tgg(i,i)
c         do j=i+1,4
c            do ii=1,4
c               do jj=ii+1,4
c                  Tgg4(i,j,ii,jj)=Tgg(i,j)*Tgg(ii,jj)
c                  Tgg4(i,i,ii,jj)=Tgg(i,i)*Tgg(ii,jj)
c                  Tgg4(i,j,ii,ii)=Tgg(i,j)*Tgg(ii,ii)
c               enddo
c            enddo
c         enddo
c      enddo
...
c     It seems like this term was missing before (but is not used finally, like Tgg4(3,4,1,2)) !!!
      Tgg4(1,2,3,4)=Tgg4(3,4,1,2)
c
