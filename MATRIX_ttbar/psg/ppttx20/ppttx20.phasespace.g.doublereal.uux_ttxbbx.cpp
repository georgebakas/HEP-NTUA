...
void ppttx20_ag_psp_400_uux_ttxbbx(int no_ps, int zero, phasespace_set & psi){
  static Logger logger("ppttx20_ag_psp_400_uux_ttxbbx");
  ...
  {psi_g_p[no_ps][  3] = psi.g_propagator(no_ps,   5,  44, psi_v_smin[no_ps][  3], psi_v_smax[no_ps][  3]); /* npp 3 */} // 1 times
  {psi_g_p[no_ps][  4] = psi.g_propagator(no_ps,   5,  28, psi_v_smin[no_ps][  4], psi_v_smax[no_ps][  4]); /* npp 3 */} // 1 times
  ...
  //  vector<double> g_t(2);
  {psi_g_t[no_ps][  0] = psi.g_tchannel_opt(no_ps,   0,  60,   1,   2,  12,  48); /* ntp 0 */} // 1 times
  {psi_g_t[no_ps][  1] = psi.g_tchannel_opt(no_ps,   0,  60,   1,   2,  48,  12); /* ntp 0 */} // 1 times
  ...
  if (psi_weight_IS == 2 || psi_weight_IS == 4){
    ...
    {psi.inv_propagator(no_ps,   5,  44, psi_v_smin[no_ps][  3], psi_v_smax[no_ps][  3], inv_r[0][3]);} // 1 times
    {psi.inv_propagator(no_ps,   5,  28, psi_v_smin[no_ps][  4], psi_v_smax[no_ps][  4], inv_r[0][4]);} // 1 times
    ...
    {psi.inv_tchannel_opt(no_ps,   0,  60,   1,   2,  12,  48, inv_r[2][0], inv_r[2][1]);} // 1 times
    {psi.inv_tchannel_opt(no_ps,   0,  60,   1,   2,  48,  12, inv_r[2][2], inv_r[2][3]);} // 1 times
...
/*
  ...
  if (psi_MC_alpha[   1] != 0.){ag_channel_4yyv(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   0, psi);}
  if (psi_MC_alpha[   2] != 0.){ag_channel_4yyv(g, 1, 2, 32, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   0, psi);}
  ...
  if (psi_MC_alpha[   5] != 0.){ag_channel_2v2v(g, 1, 2, 4, 8, 16, 32, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   0, psi);}
  if (psi_MC_alpha[   6] != 0.){ag_channel_2v2v(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   0, psi);}
  // channel    0: 4yyv(1, 2, 4, 8, 16, 32;   0,   6,   0)
  // channel    1: 4yyv(1, 2, 16, 32, 4, 8;   0,   5,   0)
  // channel    2: 4yyv(1, 2, 32, 16, 4, 8;   0,   5,   0)
  // channel    3: 4yyv(1, 2, 8, 4, 16, 32;   0,   6,   0)
  // channel    4: 42vv(1, 2, 4, 8, 16, 32;   0,   0,   0)
  // channel    5: 2v2v(1, 2, 4, 8, 16, 32;   0,   0,   0)
  // channel    6: 2v2v(1, 2, 16, 32, 4, 8;   0,   0,   0)
*/
