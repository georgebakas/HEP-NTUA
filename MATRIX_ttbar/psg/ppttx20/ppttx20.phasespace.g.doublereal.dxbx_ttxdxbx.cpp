...
void ppttx20_ag_psp_400_dxbx_ttxdxbx(int no_ps, int zero, phasespace_set & psi){
  static Logger logger("ppttx20_ag_psp_400_dxbx_ttxdxbx");
  ...
  {psi_g_p[no_ps][  2] = psi.g_propagator(no_ps,   5,  44, psi_v_smin[no_ps][  2], psi_v_smax[no_ps][  2]); /* npp 3 */} // 1 times
  ...
  if (psi_weight_IS == 2 || psi_weight_IS == 4){
    ...
    {psi.inv_propagator(no_ps,   5,  44, psi_v_smin[no_ps][  2], psi_v_smax[no_ps][  2], inv_r[0][2]);} // 1 times
...
/*
  ...
  if (psi_MC_alpha[   5] != 0.){ag_channel_13yv(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   0, psi);}
  if (psi_MC_alpha[   6] != 0.){ag_channel_112v(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   5, psi);}
  ...
  // channel    5: 13yv(1, 2, 16, 32, 4, 8;   0,   5,   0)
  // channel    6: 112v(1, 2, 16, 32, 4, 8;   0,   0,   5)
*/
