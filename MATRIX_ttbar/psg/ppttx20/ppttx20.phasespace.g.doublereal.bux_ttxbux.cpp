...
void ppttx20_ag_psp_400_bux_ttxbux(int no_ps, int zero, phasespace_set & psi){
  static Logger logger("ppttx20_ag_psp_400_bux_ttxbux");
  ...
  {psi_g_p[no_ps][  1] = psi.g_propagator(no_ps,   5,  28, psi_v_smin[no_ps][  1], psi_v_smax[no_ps][  1]); /* npp 3 */} // 1 times
  ...
  if (psi_weight_IS == 2 || psi_weight_IS == 4){
    ...
    {psi.inv_propagator(no_ps,   5,  28, psi_v_smin[no_ps][  1], psi_v_smax[no_ps][  1], inv_r[0][1]);} // 1 times
