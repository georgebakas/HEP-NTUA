...
void ppttx20_ag_psp_200_gg_ttx(int no_ps, int zero, phasespace_set & psi){
  static Logger logger("ppttx20_ag_psp_200_gg_ttx");
  ...
  //  vector<double> g_t(2);
  {psi_g_t[no_ps][  0] = psi.g_tchannel_opt(no_ps,   6,  12,   1,   2,   4,   8); /* ntp 0 */} // 1 times
  {psi_g_t[no_ps][  1] = psi.g_tchannel_opt(no_ps,   6,  12,   1,   2,   8,   4); /* ntp 0 */} // 1 times
  ...
  if (psi_weight_IS == 2 || psi_weight_IS == 4){
    ...
    {psi.inv_tchannel_opt(no_ps,   6,  12,   1,   2,   4,   8, inv_r[2][0], inv_r[2][1]);} // 1 times
    {psi.inv_tchannel_opt(no_ps,   6,  12,   1,   2,   8,   4, inv_r[2][2], inv_r[2][3]);} // 1 times

    {psi.inv_decay(no_ps,  12,   4,   8, inv_r[3][0], inv_r[3][1]);} // 1 times

    if (psi_weight_IS == 2){
      int n_random = 3 * psi_n_particle - 4;;
      vector<vector<double> > inv_channel_r(3, vector<double> (2));
      vector<double> g_IS(3, 1.);
      ...
      if (psi_MC_alpha[zero +   0] != 0.){psi_MC_g_channel[zero +   0] = g_IS[  0] * psi_g_t[no_ps][  0];}
      if (psi_MC_alpha[zero +   1] != 0.){psi_MC_g_channel[zero +   1] = g_IS[  1] * psi_g_t[no_ps][  1];}
      if (psi_MC_alpha[zero +   2] != 0.){psi_MC_g_channel[zero +   2] = g_IS[  2] * psi_g_d[no_ps][  0];}
      ...
      if (psi_MC_alpha[zero +   0] != 0.){psi_MC_g_channel[zero +   0] = psi_g_t[no_ps][  0] * g_IS[2][  0];}
      if (psi_MC_alpha[zero +   1] != 0.){psi_MC_g_channel[zero +   1] = psi_g_t[no_ps][  1] * g_IS[2][  1];}
      if (psi_MC_alpha[zero +   2] != 0.){psi_MC_g_channel[zero +   2] = psi_g_d[no_ps][  0] * g_IS[3][  0];}
    }
  }

  else {
    if (psi_MC_alpha[zero +   0] != 0.){psi_MC_g_channel[zero +   0] = psi_g_t[no_ps][  0];}
    if (psi_MC_alpha[zero +   1] != 0.){psi_MC_g_channel[zero +   1] = psi_g_t[no_ps][  1];}
    if (psi_MC_alpha[zero +   2] != 0.){psi_MC_g_channel[zero +   2] = psi_g_d[no_ps][  0];}
  }

/*
  if (psi_MC_alpha[   0] != 0.){ag_channel_11(g, 1, 2, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   6, psi);}
  if (psi_MC_alpha[   1] != 0.){ag_channel_11(g, 1, 2, 8, 4, p, s, sqrts, smin_opt, sqrtsmin_opt,   6, psi);}
  if (psi_MC_alpha[   2] != 0.){ag_channel_2v(g, 1, 2, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0, psi);}
  // channel    0: 11(1, 2, 4, 8;   6)
  // channel    1: 11(1, 2, 8, 4;   6)
  // channel    2: 2v(1, 2, 4, 8;   0)
*/
