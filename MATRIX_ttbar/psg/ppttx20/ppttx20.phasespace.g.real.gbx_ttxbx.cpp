...
void ppttx20_ag_psp_300_gbx_ttxbx(int no_ps, int zero, phasespace_set & psi){
  static Logger logger("ppttx20_ag_psp_300_gbx_ttxbx");
  ...
  {psi_g_t[no_ps][  5] = psi.g_tchannel_opt(no_ps,   5,  28,   1,   2,  16,  12); /* ntp 0 */} // 1 times
  ...
  if (psi_weight_IS == 2 || psi_weight_IS == 4){
    ...
    {psi.inv_tchannel_opt(no_ps,   5,  28,   1,   2,  16,  12, inv_r[2][10], inv_r[2][11]);} // 1 times
...
/*
  ...
  if (psi_MC_alpha[   3] != 0.){ag_channel_3yv(g, 1, 2, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5, psi);}
