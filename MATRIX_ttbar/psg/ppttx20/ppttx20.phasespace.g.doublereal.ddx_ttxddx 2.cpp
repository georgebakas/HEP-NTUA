...
void ppttx20_ag_psp_400_ddx_ttxddx(int no_ps, int zero, phasespace_set & psi){
  static Logger logger("ppttx20_ag_psp_400_ddx_ttxddx");
  ...
  {psi_g_p[no_ps][  1] = psi.g_propagator(no_ps,   0,  28, psi_v_smin[no_ps][  1], psi_v_smax[no_ps][  1]); /* npp 3 */} // 2 times
  ...
  {psi_g_p[no_ps][  4] = psi.g_propagator(no_ps,   0,  44, psi_v_smin[no_ps][  4], psi_v_smax[no_ps][  4]); /* npp 3 */} // 2 times
  ...
  {psi_g_t[no_ps][  7] = psi.g_tchannel_opt(no_ps,   0,  60,   1,   2,  12,  48); /* ntp 0 */} // 1 times
  {psi_g_t[no_ps][  8] = psi.g_tchannel_opt(no_ps,   0,  60,   1,   2,  48,  12); /* ntp 0 */} // 1 times
  ...
  if (psi_weight_IS == 2 || psi_weight_IS == 4){
    ...
    {psi.inv_propagator(no_ps,   0,  28, psi_v_smin[no_ps][  1], psi_v_smax[no_ps][  1], inv_r[0][1]);} // 2 times
    ...
    {psi.inv_propagator(no_ps,   0,  44, psi_v_smin[no_ps][  4], psi_v_smax[no_ps][  4], inv_r[0][4]);} // 2 times
    ...
    {psi.inv_tchannel_opt(no_ps,   0,  60,   1,   2,  12,  48, inv_r[2][14], inv_r[2][15]);} // 1 times
    {psi.inv_tchannel_opt(no_ps,   0,  60,   1,   2,  48,  12, inv_r[2][16], inv_r[2][17]);} // 1 times
...
/*
  ...
  if (psi_MC_alpha[   5] != 0.){ag_channel_4yyv(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   0, psi);}
  if (psi_MC_alpha[   6] != 0.){ag_channel_4yyv(g, 1, 2, 32, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   0, psi);}
  ...
  if (psi_MC_alpha[  11] != 0.){ag_channel_112v(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   0, psi);}
  if (psi_MC_alpha[  12] != 0.){ag_channel_2v2v(g, 1, 2, 4, 8, 16, 32, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   0, psi);}
  if (psi_MC_alpha[  13] != 0.){ag_channel_2v2v(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   0, psi);}
  ...
  // channel    5: 4yyv(1, 2, 16, 32, 4, 8;   0,   0,   0)
  // channel    6: 4yyv(1, 2, 32, 16, 4, 8;   0,   0,   0)
  ...
  // channel   11: 112v(1, 2, 16, 32, 4, 8;   0,   0,   0)
  // channel   12: 2v2v(1, 2, 4, 8, 16, 32;   0,   0,   0)
  // channel   13: 2v2v(1, 2, 16, 32, 4, 8;   0,   0,   0)
*/
