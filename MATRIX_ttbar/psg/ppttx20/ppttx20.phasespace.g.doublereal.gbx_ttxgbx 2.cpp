...
void ppttx20_ag_psp_400_gbx_ttxgbx(int no_ps, int zero, phasespace_set & psi){
  static Logger logger("ppttx20_ag_psp_400_gbx_ttxgbx");
  ...
  {psi_g_p[no_ps][  4] = psi.g_propagator(no_ps,   5,  44, psi_v_smin[no_ps][  4], psi_v_smax[no_ps][  4]); /* npp 3 */} // 3 times
  ...
  {psi_g_p[no_ps][  7] = psi.g_propagator(no_ps,   5,  48, psi_v_smin[no_ps][  7], psi_v_smax[no_ps][  5]); /* npp 2 */} // 4 times
  {psi_g_p[no_ps][  8] = psi.g_propagator(no_ps,   0,  12, psi_v_smin[no_ps][  0], psi_v_smax[no_ps][  6]); /* npp 2 */} // 2 times
  {psi_g_p[no_ps][  9] = psi.g_propagator(no_ps,   5,  48, psi_v_smin[no_ps][  7], psi_v_smax[no_ps][  7]); /* npp 2 */} // 1 times
  ...
  {psi_g_t[no_ps][ 12] = psi.g_tchannel_opt(no_ps,   5,  52,   2,   9,  16,  36); /* ntp 1 */} // 1 times
  ...
  {psi_g_t[no_ps][ 18] = psi.g_tchannel_opt(no_ps,   5,  60,   2,   1,  16,  44); /* ntp 0 */} // 2 times
  ...
  {psi_g_t[no_ps][ 22] = psi.g_tchannel_opt(no_ps,   5,  60,   1,   2,  32,  28); /* ntp 0 */} // 5 times
  {psi_g_t[no_ps][ 23] = psi.g_tchannel_opt(no_ps,   5,  28,   2,  33,  16,  12); /* ntp 1 */} // 1 times
  {psi_g_t[no_ps][ 24] = psi.g_tchannel_opt(no_ps,   0,  56,   2,   5,  48,   8); /* ntp 1 */} // 1 times
  {psi_g_t[no_ps][ 25] = psi.g_tchannel_opt(no_ps,   5,  44,   2,  17,  12,  32); /* ntp 1 */} // 1 times
  {psi_g_t[no_ps][ 26] = psi.g_tchannel_opt(no_ps,   5,  28,   2,  33,  12,  16); /* ntp 1 */} // 1 times
  ...
  {psi_g_t[no_ps][ 29] = psi.g_tchannel_opt(no_ps,   5,  60,   1,   2,  48,  12); /* ntp 0 */} // 1 times
  ...
  if (psi_weight_IS == 2 || psi_weight_IS == 4){
    ...
    {psi.inv_propagator(no_ps,   5,  44, psi_v_smin[no_ps][  4], psi_v_smax[no_ps][  4], inv_r[0][4]);} // 3 times
    ...
    {psi.inv_propagator(no_ps,   5,  48, psi_v_smin[no_ps][  7], psi_v_smax[no_ps][  5], inv_r[0][7]);} // 4 times
    {psi.inv_propagator(no_ps,   0,  12, psi_v_smin[no_ps][  0], psi_v_smax[no_ps][  6], inv_r[0][8]);} // 2 times
    {psi.inv_propagator(no_ps,   5,  48, psi_v_smin[no_ps][  7], psi_v_smax[no_ps][  7], inv_r[0][9]);} // 1 times
    ...
    {psi.inv_tchannel_opt(no_ps,   5,  52,   2,   9,  16,  36, inv_r[2][24], inv_r[2][25]);} // 1 times
    ...
    {psi.inv_tchannel_opt(no_ps,   5,  60,   2,   1,  16,  44, inv_r[2][36], inv_r[2][37]);} // 2 times
    ...
    {psi.inv_tchannel_opt(no_ps,   5,  60,   1,   2,  32,  28, inv_r[2][44], inv_r[2][45]);} // 5 times
    {psi.inv_tchannel_opt(no_ps,   5,  28,   2,  33,  16,  12, inv_r[2][46], inv_r[2][47]);} // 1 times
    {psi.inv_tchannel_opt(no_ps,   0,  56,   2,   5,  48,   8, inv_r[2][48], inv_r[2][49]);} // 1 times
    {psi.inv_tchannel_opt(no_ps,   5,  44,   2,  17,  12,  32, inv_r[2][50], inv_r[2][51]);} // 1 times
    {psi.inv_tchannel_opt(no_ps,   5,  28,   2,  33,  12,  16, inv_r[2][52], inv_r[2][53]);} // 1 times
    ...
    {psi.inv_tchannel_opt(no_ps,   5,  60,   1,   2,  48,  12, inv_r[2][58], inv_r[2][59]);} // 1 times
...
/*
  ...
  if (psi_MC_alpha[  13] != 0.){ag_channel_3yv1(g, 1, 2, 32, 4, 8, 16, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   5, psi);}
  ...
  if (psi_MC_alpha[  16] != 0.){ag_channel_4yyv(g, 1, 2, 32, 4, 8, 16, p, s, sqrts, smin_opt, sqrtsmin_opt,   6,   0,   5, psi);}
  if (psi_MC_alpha[  17] != 0.){ag_channel_4yyv(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   5, psi);}
  if (psi_MC_alpha[  18] != 0.){ag_channel_4yyv(g, 1, 2, 32, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   5, psi);}
  if (psi_MC_alpha[  19] != 0.){ag_channel_4yyv(g, 1, 2, 32, 8, 4, 16, p, s, sqrts, smin_opt, sqrtsmin_opt,   6,   0,   5, psi);}
  if (psi_MC_alpha[  20] != 0.){ag_channel_42vv(g, 1, 2, 4, 8, 16, 32, p, s, sqrts, smin_opt, sqrtsmin_opt,   5,   0,   5, psi);}
  if (psi_MC_alpha[  21] != 0.){ag_channel_12v1(g, 1, 2, 4, 8, 16, 32, p, s, sqrts, smin_opt, sqrtsmin_opt,   6,   6,   0, psi);}
  if (psi_MC_alpha[  22] != 0.){ag_channel_12v1(g, 1, 2, 32, 4, 8, 16, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   5, psi);}
  ...
  if (psi_MC_alpha[  26] != 0.){ag_channel_13yv(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   0, psi);}
  if (psi_MC_alpha[  27] != 0.){ag_channel_13yv(g, 1, 2, 32, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   5, psi);}
  if (psi_MC_alpha[  28] != 0.){ag_channel_13yv(g, 1, 2, 32, 8, 4, 16, p, s, sqrts, smin_opt, sqrtsmin_opt,   6,   0,   5, psi);}
  if (psi_MC_alpha[  29] != 0.){ag_channel_112v(g, 1, 2, 4, 8, 16, 32, p, s, sqrts, smin_opt, sqrtsmin_opt,   5,   6,   0, psi);}
  if (psi_MC_alpha[  30] != 0.){ag_channel_112v(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   0,   5, psi);}
  if (psi_MC_alpha[  31] != 0.){ag_channel_112v(g, 1, 2, 32, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   5, psi);}
  if (psi_MC_alpha[  32] != 0.){ag_channel_112v(g, 1, 2, 8, 4, 16, 32, p, s, sqrts, smin_opt, sqrtsmin_opt,   5,   6,   0, psi);}
  if (psi_MC_alpha[  33] != 0.){ag_channel_2v2v(g, 1, 2, 4, 8, 16, 32, p, s, sqrts, smin_opt, sqrtsmin_opt,   5,   0,   0, psi);}
  if (psi_MC_alpha[  34] != 0.){ag_channel_2v2v(g, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   5, psi);}
  ...
  // channel   13: 3yv1(1, 2, 32, 4, 8, 16;   0,   5,   5)
  // channel   14: 3yv1(1, 2, 16, 4, 8, 32;   0,   0,   0)
  // channel   15: 3yv1(1, 2, 8, 4, 16, 32;   6,   0,   0)
  // channel   16: 4yyv(1, 2, 32, 4, 8, 16;   6,   0,   5)
  // channel   17: 4yyv(1, 2, 16, 32, 4, 8;   0,   5,   5)
  // channel   18: 4yyv(1, 2, 32, 16, 4, 8;   0,   0,   5)
  // channel   19: 4yyv(1, 2, 32, 8, 4, 16;   6,   0,   5)
  // channel   20: 42vv(1, 2, 4, 8, 16, 32;   5,   0,   5)
  // channel   21: 12v1(1, 2, 4, 8, 16, 32;   6,   6,   0)
  // channel   22: 12v1(1, 2, 32, 4, 8, 16;   0,   5,   5)
  // channel   23: 12v1(1, 2, 16, 4, 8, 32;   0,   0,   0)
  // channel   24: 12v1(1, 2, 8, 4, 16, 32;   6,   6,   0)
  // channel   25: 13yv(1, 2, 32, 4, 8, 16;   6,   0,   5)
  // channel   26: 13yv(1, 2, 16, 32, 4, 8;   0,   5,   0)
  // channel   27: 13yv(1, 2, 32, 16, 4, 8;   0,   0,   5)
  // channel   28: 13yv(1, 2, 32, 8, 4, 16;   6,   0,   5)
  // channel   29: 112v(1, 2, 4, 8, 16, 32;   5,   6,   0)
  // channel   30: 112v(1, 2, 16, 32, 4, 8;   0,   0,   5)
  // channel   31: 112v(1, 2, 32, 16, 4, 8;   0,   5,   5)
  // channel   32: 112v(1, 2, 8, 4, 16, 32;   5,   6,   0)
  // channel   33: 2v2v(1, 2, 4, 8, 16, 32;   5,   0,   0)
  // channel   34: 2v2v(1, 2, 16, 32, 4, 8;   0,   5,   5)
*/
