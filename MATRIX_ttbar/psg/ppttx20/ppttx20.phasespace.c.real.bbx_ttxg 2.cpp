...
void ppttx20_ac_psp_300_bbx_ttxg(int no_ps, int channel, phasespace_set & psi){
  static Logger logger("ppttx20_ac_psp_300_bbx_ttxg");
  ...
  for (int i_t = 0; i_t < psi_c_t[no_ps][channel].size(); i_t++){
    if      (psi_c_t[no_ps][channel][i_t] ==   0){
      ...
      psi.c_tchannel_opt(no_ps,   5,  28,   2,   1,  16,  12, ran + 2 * i_t, ran + 2 * i_t + 1);
    }
      ...
      psi.c_tchannel_opt(no_ps,   5,  28,   1,   2,  16,  12, ran + 2 * i_t, ran + 2 * i_t + 1);
    }
  }
  ...
  /*
  if      (channel ==    0){ac_channel_2v1(r, 1, 2, 4, 8, 16, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5, psi);}
  ...
  else if (channel ==    4){ac_channel_12v(r, 1, 2, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5, psi);}
  */
