...
void ppttx20_ac_psp_400_uux_ttxbbx(int no_ps, int channel, phasespace_set & psi){
  static Logger logger("ppttx20_ac_psp_400_uux_ttxbbx");
  ...
  for (int i_p = 0; i_p < psi_c_p[no_ps][channel].size(); i_p++){
    if (psi_c_p[no_ps][channel][i_p] ==   0){
      ...
      psi.c_propagator(no_ps,   5,  44, psi_v_smin[no_ps][  3], psi_v_smax[no_ps][  3], ran + i_p);
    }
  ...
  else if (channel ==    1){ac_channel_4yyv(r, 1, 2, 16, 32, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   0, psi);}
  else if (channel ==    2){ac_channel_4yyv(r, 1, 2, 32, 16, 4, 8, p, s, sqrts, smin_opt, sqrtsmin_opt,   0,   5,   0, psi);}
