...
CC
      ...
      double precision tH1stFg, tH1stFq,tH1stFqqb,tH1stFqbq
     ...
     .                 C2ggreg
      ...
      double precision H1ggdelta,H1qqdelta,H2gdelta
...
c javi: adding definitions for h and for 2-loop virtuals
      double precision h1_TT,h2_TT,h2_TTTT
      double precision virtgg,virtqq
      double precision javi_H1ggdelta,javi_H1qqdelta,javi_H2ggdelta,
     &                 javi_H2qqdelta
      double precision H2qdelta,H2qbdelta,tdeltaqqb,tdeltaqbq
...
ch
C    call to H2, only in the case it is needed
      if (order.eq.2.and.fullH2) then
        if (.not.noglue) then
          H2ggdelta = javi_H2ggdelta(p)
        endif
        if (.not.ggonly) then
          !H2qqdelta = javi_H2qqdelta(p)
          H2qdelta  = javi_H2qqdelta(p)
          H2qbdelta = javi_H2qqdelta(pb)
        endif
      endif
      tdeltaqqb = 0.d0
      tdeltaqbq = 0.d0
...
C     Simplest term without convolutions
      ...
      if(j.gt.0.and.k.lt.0)then
        tdeltaqqb=tdeltaqqb+fx10(j)*fx20(k)*msqc(j,k)*flgq
      elseif(j.lt.0.and.k.gt.0)then
        tdeltaqbq=tdeltaqbq+fx10(j)*fx20(k)*msqc(j,k)*flgq
      endif
...
c JAVI: added
      if(j.gt.0)then
      tH1stFqqb=tH1stFqqb+diff*fx20(k)*msqc(j,k)
      tH1stFqqb=tH1stFqqb-Pqqint(xx10)*fx10(j)*fx20(k)*msqc(j,k)*flgq
      else
      tH1stFqbq=tH1stFqbq+diff*fx20(k)*msqc(j,k)
      tH1stFqbq=tH1stFqbq-Pqqint(xx10)*fx10(j)*fx20(k)*msqc(j,k)*flgq
      endif
...
c JAVI: added
      ...
      if(j.gt.0)then
      tH1stFqqb=tH1stFqqb+diff*fx10(j)*msqc(j,k)
      tH1stFqqb=tH1stFqqb-Pqqint(xx20)*fx10(j)*fx20(k)*msqc(j,k)*flgq
      else
      tH1stFqbq=tH1stFqbq+diff*fx10(j)*msqc(j,k)
      tH1stFqbq=tH1stFqbq-Pqqint(xx20)*fx10(j)*fx20(k)*msqc(j,k)*flgq
      endif
...
c JAVI: replaced by the lines below
c      tH2st=tH2st+fx1p(j)*Cqq(z1)*(-dlog(xx10))*
c     &            fx20(k)*H1qdelta*msqc(j,k)*flgq*flqq       
c
c      tH2st=tH2st+fx2p(k)*Cqq(z2)*(-dlog(xx20))*
c     &            fx10(j)*H1qdelta*msqc(j,k)*flgq*flqq        
     ...
     &            fx10(j)*H1qdelta*msqc(j,k)*flgq*flqq        
      elseif(k.gt.0.and.j.lt.0)then
      tH2st=tH2st+fx1p(j)*Cqq(z1)*(-dlog(xx10))*
     &            fx20(k)*H1qbdelta*msqc(j,k)*flgq*flqq       
      tH2st=tH2st+fx2p(k)*Cqq(z2)*(-dlog(xx20))*
     &            fx10(j)*H1qbdelta*msqc(j,k)*flgq*flqq        
      endif
...
c JAVI: replaced by the lines below
c      tH2st=tH2st+fx1p(0)*Cqg(z1)*(-dlog(xx10))*
c     &            fx20(k)*H1qdelta*msqc(j,k)*flqq       
c
c      tH2st=tH2st+fx2p(0)*Cqg(z2)*(-dlog(xx20))*
c     &            fx10(j)*H1qdelta*msqc(j,k)*flqq
     ...
     &            fx10(j)*H1qdelta*msqc(j,k)*flqq
      elseif(k.gt.0.and.j.lt.0)then
      tH2st=tH2st+fx1p(0)*Cqg(z1)*(-dlog(xx10))*
     &            fx20(k)*H1qbdelta*msqc(j,k)*flqq       
      tH2st=tH2st+fx2p(0)*Cqg(z2)*(-dlog(xx20))*
     &            fx10(j)*H1qbdelta*msqc(j,k)*flqq
      endif
...
ch     &          +asopi*(tH1st+LF*tH1stF)
     &         +asopi**2*(
c     &                      tdeltaqq*H2qqdelta*flqq !JAVI: original, replaced by the two lines below
     &                      tdeltaqqb*H2qdelta*flqq
     &                     +tdeltaqbq*H2qbdelta*flqq
...
c     &               -2*beta0*LR*H1qdelta*tdeltaqq*flqq !JAVI: original, replaced by the two lines below
     &               -2*beta0*LR*H1qdelta*tdeltaqqb*flqq
     &               -2*beta0*LR*H1qbdelta*tdeltaqbq*flqq
     &               -2*beta0*LR*H1gdelta*tdeltagg*flqq
     &              )
cch

      xmsq=xmsq+asopi**2*(LF*H1gdelta*tH1stFg)*flqq
c      xmsq=xmsq+asopi**2*(LF*H1qdelta*tH1stFq)*flqq ! JAVI: original, replaced by the two lines below
      xmsq=xmsq+asopi**2*(LF*H1qdelta*tH1stFqqb)*flqq
      xmsq=xmsq+asopi**2*(LF*H1qbdelta*tH1stFqbq)*flqq
...
ch
     ...
     .         +dlog(scale**2/mt**2)*PoleEp1
      ...
      double precision function javi_H1ggdelta(p)
      use openloops
      implicit none
      include 'masses.f'
      include 'constants.f'
      ...
      double precision FacDif,Add,PoleEp1,PoleEp2
      double precision M0T34eps,M0T13eps,M0T23eps,M0sqeps
      double precision sn,tn,un,FinVirtgg,beta,cost
      double precision h1_TT,h2_TT,h2_TTTT
      double precision virtgg,Ci,xnf,b0
      double precision ol_p(0:3,4),m0_OL,m2l1(0:2),acc,m2cc(4,4)
      double precision test,PoleEPm2gg,PoleEPm1gg,PoleEP0gg,M1M0
      ...
      sn=2d0*dot(p,1,2)
      tn=2d0*dot(p,1,3)+mt**2.d0
      un=2d0*dot(p,2,3)+mt**2.d0

      call set_parameter("alphas", as)
      call set_parameter("mu",dsqrt(sn))
      call evaluate_ccmatrix(ol_id(0,0), ol_p, m0_OL, m2cc, acc)
      call evaluate_loop(ol_id(0,0), ol_p, m0_OL, m2l1, acc)

      beta = dsqrt(1.d0-4.d0*mt**2.d0/sn)
      cost = (tn-un)/sn/beta

      Ci = CA

      M1M0 = m2l1(0)/(as/(2.d0*pi))
C The convention from Massimiliano is different from the one in
C OpenLoops, so I need to fix that. It is a term:
      M1M0 = M1M0 + 2.d0*Ci*pi**2.d0/12.d0*m0_OL

      javi_H1ggdelta = M1M0
     &               + m0_OL*(-(Ci*Pi**2)/6. 
     &        + 2.d0*CF*h1_TT(3,3,beta,cost))
     &        + 2.d0*h1_TT(3,4,beta,cost)*m2cc(3,4)

c I add a factor 1/2 because Hayk's expansion is in powers
c of as/pi, while our formula is for as/(2*pi)
      javi_H1ggdelta=javi_H1ggdelta/2.d0

c I add the scale dependent terms
      xnf = dble(nf)
      b0=(11.d0/2.d0-xnf/3.d0)
      javi_H1ggdelta=javi_H1ggdelta-b0*dlog(sn/scale**2.d0)*m0_OL

      return
      end


      double precision function javi_H1qqdelta(p)
      use openloops
      implicit none
      include 'masses.f'
      include 'constants.f'
      ...
      double precision sn,tn,un,FinVirtgg,beta,cost
      double precision h1_TT,h2_TT,h2_TTTT
      double precision virtgg,Ci,xnf,b0,M1M0
      double precision ol_p(0:3,4),m0_OL,m2l1(0:2),acc,m2cc(4,4)
      double precision test,PoleEPm2gg,PoleEPm1gg,PoleEP0gg
      ...
      ! Checked that the OL LO agrees with ggQQv_0*avegg
      call set_parameter("alphas", as)
      call set_parameter("mu",dsqrt(sn))
      !call evaluate_tree(ol_id(0,0), ol_p, m0_OL)
      call evaluate_ccmatrix(ol_id(1,-1), ol_p, m0_OL, m2cc, acc)
      call evaluate_loop(ol_id(1,-1), ol_p, m0_OL, m2l1, acc)
...
C The convention from Massimiliano is different from the one in
C OpenLoops, so I need to fix that. It is a term:
      M1M0 = M1M0 + 2.d0*Ci*pi**2.d0/12.d0*m0_OL !Check factor 2!!!
...
c I add a factor 1/2 because Hayk's expansion is in powers
c of as/pi, while our formula is for as/(2*pi)
      javi_H1qqdelta=javi_H1qqdelta/2.d0

c To match Hayk's definition of H1qqdelta I add a CF*pi^2/12*M02
c      javi_H1qqdelta = javi_H1qqdelta + Ci*pi**2./12.d0*m0_OL

c I add the scale dependent terms
      xnf = dble(nf)
      b0=(11.d0/2.d0-xnf/3.d0)
      javi_H1qqdelta=javi_H1qqdelta-b0*dlog(sn/scale**2.d0)*m0_OL



C ========================================================
C ========================================================
C comparision between my implementation and Hayk's

C The object defined as

c      test =   m0_OL*(-(CA*Pi**2)/6. 
c     &        + 2.d0*CF*h1_TT(3,3,beta,cost))
c     &        + 2.d0*h1_TT(3,4,beta,cost)*Tgg(3,4)*m0_OL
c      test=test/2.d0

C is equal to the output of the H1ggdelta function if I include
C only the FacDif contribution and if I kill the terms that
C are added under the comment 
c "New Finite term coming from ep^2 term of the Born"
C So essentially I have a factor 1/2 that I don't know where
C it comes from and I have those additional terms that I
C do not understand either

C Checked that the pole of FinVirtgg(sn,tn)*gsq**2/4d0*avegg
C is equal to m2l1(2)/(as/pi)

C The objects PoleEp2*avegg and PoleEp1*avegg agree with
C m2l1(2)/(as/pi) and m2l1(1)/(as/pi) if I set
C set_parameter("mu",mt)
C OJO: me olvide de copiar un termino de PoleEp1!!!!!!!!!

C Checked all 2-particle color correlators.
C (checked only the ones directly provided by OpenLoops, so
C didn't check explicitly the ones that can be obtained
C from them).
C All of them work EXCEPT for Tgg(1,2), which however
C doesn't seem to be used in the code. Fixed typo in Tgg(1,2)
C now all of them work
...
C ========================================================
      ...
      double precision function javi_H2ggdelta(p)
      use openloops
      implicit none
      include 'masses.f'
      include 'constants.f'
      ...
      double precision sn,tn,un,FinVirtgg,beta,cost
      double precision h1_TT,h2_TT,h2_TTTT
      double precision virtgg,Ci
      double precision ol_p(0:3,4),m0_OL,m2l1(0:2),acc,m1sq
      double precision M2M0,M1M0,M1M1,M1M0cc(4,4),M0M0cc(4,4),
     &                 M0M0_3434,M0M0,M0M04c(4,4,4,4),
     &                 imM1M0,imM1M0cc(4,4),m2l1_im(0:2)
      double precision dummy,fac,b0,b1,xnf,test,facgg
      double precision hayk_M1M0,hayk_M1M0cc34
      double precision gama1,gama2,logM2mt2,vv,KK,gamaN,aux
     ...
     &               aux2C,M,Mcc(4,4),aux3C
      ...
      call set_parameter("alphas", as)
      call set_parameter("mu",dsqrt(sn))
      call evaluate_ccmatrix(ol_id(0,0), ol_p, m0_OL, M0M0cc, acc)
      call evaluate_loop(ol_id(0,0), ol_p, dummy, m2l1, acc)
      call evaluate_loop(ol_id_im(0,0), ol_p, dummy, m2l1_im, acc)
      call evaluate_loop2(ol_id_m1sq(0,0), ol_p, M1M1, acc)
      call evaluate_loopccmatrix(ol_id_m1sq(0,0)
     &                       , ol_p, dummy, M1M0cc, acc)
      call evaluate_loopccmatrix(ol_id_m1sq_im(0,0)
     &                       , ol_p, dummy, imM1M0cc, acc)


C The M1*M0 + cc I can take from OpenLoops, also the cc ones:
      M1M0 = m2l1(0)/(as/(2.d0*pi))
      imM1M0 = m2l1_im(0)/(as/(2.d0*pi))
      do i=1,4
      do j=1,4
        M1M0cc(i,j)=M1M0cc(i,j)/(as/(2.d0*pi))
        imM1M0cc(i,j)=imM1M0cc(i,j)/(as/(2.d0*pi))
      enddo
      enddo

C The M1*M1 I also take from OL
      M1M1 = M1M1/(as/(2.d0*pi))**2.d0

C I evaluate the M2*M0 + cc. Following equation 3.6 of
C 1312.6279, this can be obtained from the grid as
      M2M0 = virtgg(beta,cost)*4096.d0*pi
     &          /(beta*(1.d0-beta**2.d0))
...
C OpenLoops, so I need to fix that:
      ...
      fac = Ci*pi**2.d0/12.d0
      do i=1,4
      do j=1,4
        M1M0cc(i,j)=M1M0cc(i,j) + 2.d0*fac*M0M0cc(i,j)
      enddo
      enddo
      M1M1 = M1M1 + fac*M1M0 + fac**2.d0*M0M0
      M1M0 = M1M0 + 2.d0*fac*M0M0

C The result for H2:

      javi_H2ggdelta = M1M1 + M2M0 + (M1M0*(-(Ci*Pi**2.)/3.d0
     &  + 4.d0*CF*h1_TT(3,3,beta,cost)))/
     &  2.d0 + M0M0*((Ci**2.*Pi**4.)/72.d0 + 
     &  (Ci*(CA*(4856.d0 - 603.d0*Pi**2. + 18.d0*Pi**4. - 2772.d0*z3) +
     &  xnf*(-656.d0 + 90.d0*Pi**2. + 504.d0*z3) - 
     &  216.d0*CF*Pi**2.*h1_TT(3,3,beta,cost)))/648.d0 + 
     &  CF**2.d0*(h1_TT(3,3,beta,cost)**2. 
     &  + 2.d0*h2_TTTT(3,3,3,3,beta,cost))) + 
     &  (h1_TT(3,4,beta,cost)**2. 
     &  + 2.d0*h2_TTTT(3,4,3,4,beta,cost))*M0M0_3434 + 
     &  2.d0*h2_TT(3,1,beta,cost)*M0M0cc(1,3) 
     &  + 2.d0*h2_TT(4,1,beta,cost)*M0M0cc(1,4) + 
     &  2.d0*h2_TT(3,2,beta,cost)*M0M0cc(2,3) 
     &  + 2.d0*h2_TT(4,2,beta,cost)*M0M0cc(2,4) + 
     &  (-(Ci*Pi**2*h1_TT(3,4,beta,cost))/3.d0 + 
     &  2.d0*(h2_TT(3,4,beta,cost) + CF*
     &  (h1_TT(3,3,beta,cost)*h1_TT(3,4,beta,cost) 
     &  + h2_TTTT(3,4,3,3,beta,cost))))*
     &  M0M0cc(3,4) + 2.d0*h1_TT(3,4,beta,cost)*M1M0cc(3,4)


C I add the term for the scale mismatch between mt and M in 2-loop virtuals
      b0=11.d0/2.d0-xnf/3.d0 
      b1=51.d0/2.d0-19.d0/6.d0*xnf
      logM2mt2=dlog(sn/mt**2.d0)

      javi_H2ggdelta = javi_H2ggdelta + 
     &    2.d0*logM2mt2*(b1*M0M0 + b0*M1M0 
     &           - b0**2.d0*M0M0*logM2mt2)

c      write(*,*)'antes = ',javi_H2ggdelta
c      goto 1234
...
C I start by defining gama1g and gama2g and K
      ...
      gama1 = -b0/2.d0
      gama2 = (2.d0*CF*xnf + (CA*(256.d0/27.d0
     &        - (2.d0*Pi**2.d0)/9.d0)*xnf)/2.d0
     &        + CA**2.d0*(-692.d0/27.d0 + (11.d0*Pi**2.d0)/18.d0
     &        + 2.d0*z3))/16.d0
      KK = (67.d0/18.d0-Pi**2.d0/6.d0)*CA-5.d0/9.d0*xnf
      gamaN = (20.d0*CF*xnf)/9.d0 + CA*CF*(-98.d0/9.d0
     &        + (2.d0*Pi**2.d0)/3.d0 - 4.d0*z3)

C I define v and b34
      vv = dsqrt(1.d0-mt**4.d0/dot(p,3,4)**2.d0)
      b34 = im*Pi + 1.d0/2.d0*dlog((1.d0-vv)/(1.d0+vv))
      expB34 = cdExp(-2.d0*b34)
      li2b34 = HPL2(0,1,expB34)
      li3b34 = HPL3(0,0,1,expB34)
      CothB34 = (cdExp(-b34) + cdExp(b34))/(-cdExp(-b34) + cdExp(b34))
...
C TODO CHECK IMAGINARY PART
      M = -4.d0*b0*logM2mt2*M0M0+M1M0+im*imM1M0
      do i = 1,4
      do j = 1,4
        Mcc(i,j) = -4.d0*b0*logM2mt2*M0M0cc(i,j)
     &             +M1M0cc(i,j)+im*imM1M0cc(i,j)
      enddo
      enddo

      aux1C =   (logM2mt2*(-2*CF*M + 4*gama1*M + 
     -      Ci*logM2mt2*M - 2*Ci*im*M*Pi - 
     -      2*b34*CothB34*Mcc(3,4)))/2.
      do i = 3,4
      do j = 1,2
      aux1C = aux1C + (
     &   (logM2mt2*(-(im*Pi) + 
     -      dlog((mt**2*sn)/(4.*dot(p,i,j)**2)))*Mcc(i,j))/
     -  2.
     & )
      enddo
      enddo

      javi_H2ggdelta = javi_H2ggdelta + dble(aux1C)


C I write now the term 2*Re(<M0|U2-U1^2|M0>)
      do i = 3,4
      do j = 1,2
        M0M04c(i,j,3,4) = Tgg4(j,i,3,4)*m0_OL
        do k = 3,4
        do l = 1,2
          M0M04c(i,j,k,l) = Tgg4(j,i,l,k)*m0_OL
        enddo
        enddo
      enddo
      enddo
      ...
      aux2C =  (logM2mt2*(-36.*b34**2.*CothB34**2.*logM2mt2*M0M0_3434 + 
     -      6.*M0M0*(48.*gama2 + 3.*gamaN - 6.*b0*CF*logM2mt2 + 
     -         12.*b0*gama1*logM2mt2 + 6.*Ci*KK*logM2mt2 + 
     -         4.*b0*Ci*logM2mt2**2. - 12.*Ci*im*KK*Pi - 
     -         6.*b0*Ci*im*logM2mt2*Pi) - 
     -      9.*logM2mt2*M0M0*
     -       (-2.*CF + 4.*gama1 + Ci*(logM2mt2 - 2.*im*Pi))**2. - 
     -      4.*(-18.*b34**2.*CA*(-1. + CothB34) + 
     -         6.*b34**3.*CA*(-1. + CothB34)*CothB34 + 
     -         b34*CothB34*
     -          (9.*b0*logM2mt2 + 18.*CF*logM2mt2 - 
     -            36.*gama1*logM2mt2 - 9.*Ci*logM2mt2**2. + 
     -            18.*Ci*im*logM2mt2*Pi + 
     -            CA*(67. - 6.*Pi**2. + 
     -               3.*CothB34*(6.*li2b34 + Pi**2.)) - 10.*xnf) + 
     -         3.*CA*(Pi**2. + CothB34*(6.*li2b34 - Pi**2.) + 
     -            6.*CothB34**2.*(li3b34 - z3) + 6.*z3) - 
     -         36.*b34*CA*CothB34*cdlog(1. - expB34))*M0M0cc(3,4))
     -    )/36.d0
      do i = 3,4
      do j = 1,2
      aux2C = aux2C + (
     &  (logM2mt2*(Pi*(-9.*b0*im*logM2mt2 - 18.*CF*im*logM2mt2 + 
     -          36.*gama1*im*logM2mt2 + 9.*Ci*im*logM2mt2**2. + 
     -          18.*Ci*logM2mt2*Pi + CA*im*(-67. + 3.*Pi**2.) + 
     -          10.*im*xnf) + 
     -       (9.*b0*logM2mt2 + 18.*CF*logM2mt2 - 
     -          36.*gama1*logM2mt2 - 9.*Ci*logM2mt2**2. + 
     -          18.*Ci*im*logM2mt2*Pi + CA*(67. - 3.*Pi**2.) - 
     -          10.*xnf)*dlog((mt**2.*sn)/(4.*dot(p,i,j)**2.)))*
     -     M0M0cc(i,j))/18.d0 + 
     -  b34*CothB34*logM2mt2**2.*
     -   (-(im*Pi) + dlog((mt**2.*sn)/(4.d0*dot(p,i,j)**2.)))*
     -   M0M04c(i,j,3,4)
     & )
      do k = 3,4
      do l = 1,2
      aux2C = aux2C + (
     &  (logM2mt2**2.*(Pi**2. + 
     -      dlog((mt**2.*sn)/(4.d0*dot(p,i,j)**2.))*
     -       (2.*im*Pi - dlog((mt**2.*sn)/(4.*dot(p,k,l)**2.)))))/
     -  4.d0
     & )*M0M04c(i,j,k,l)
      enddo
      enddo
      ...
      javi_H2ggdelta = javi_H2ggdelta + dble(aux2C)


c      write(*,*)'final = ',javi_H2ggdelta
C Finished adding terms due to mt->M

1234   continue

c I add a factor 1/4 because Hayk's expansion is in powers
c of as/pi, while our formula is for as/(2*pi)
      javi_H2ggdelta=javi_H2ggdelta/4.d0

C I normalize by M0M0 directly here
      javi_H2ggdelta=javi_H2ggdelta/M0M0
      ...
      double precision function javi_H2qqdelta(p)
      use openloops
      implicit none
      include 'masses.f'
      include 'constants.f'
      ...
      double precision FacDif,Add,PoleEp1,PoleEp2
      double precision M0T34eps,M0T13eps,M0T23eps,M0sqeps
      double precision sn,tn,un,beta,cost
      double precision h1_TT,h2_TT,h2_TTTT
      double precision virtqq,Ci,facqq
      double precision ol_p(0:3,4),m0_OL,m2l1(0:2),acc,m1sq
      double precision M2M0,M1M0,M1M1,M1M0cc(4,4),M0M0cc(4,4),
     &                 M0M0_3434,M0M0,imM1M0,imM1M0cc(4,4),
     &                 M0M04c(4,4,4,4),m2l1_im(0:2)
      double precision dummy,fac,b0,b1,xnf
      double precision gama1,gama2,logM2mt2,vv,KK,gamaN,aux
      double complex b34,expb34,li2b34,li3b34,HPL2,HPL3,CothB34,aux1C,
     &               aux2C,aux3C,M,Mcc(4,4)
      ...
      call set_parameter("alphas", as)
      call set_parameter("mu",dsqrt(sn))
      call evaluate_ccmatrix(ol_id(1,-1), ol_p, m0_OL, M0M0cc, acc)
      call evaluate_loop(ol_id(1,-1), ol_p, dummy, m2l1, acc)
      call evaluate_loop(ol_id_im(1,-1), ol_p, dummy, m2l1_im, acc)
      call evaluate_loop2(ol_id_m1sq(1,-1), ol_p, M1M1, acc)
      call evaluate_loopccmatrix(ol_id_m1sq(1,-1)
     &                       , ol_p, dummy, M1M0cc, acc)
      call evaluate_loopccmatrix(ol_id_m1sq_im(1,-1)
     &                       , ol_p, dummy, imM1M0cc, acc)
...
C I evaluate the M2*M0 + cc. Following equation 3.6 of
C 1312.6279, this can be obtained from the grid as
      M2M0 = virtqq(beta,cost)*576.d0*pi
     &          /(beta*(1.d0-beta**2.d0))
C where the expansion for M is done in powers of as/(2*pi).
C To be consistent with the rest, I add the overall power
C of as, which otherwise is missing:
      M2M0 = M2M0 * gsq**2.d0
C I add factors from avergage of initial state d.o.f
      facqq = 1.d0/3.d0/3.d0/2.d0/2.d0
      M2M0 = M2M0 * facqq

C Finally, the tree-level 4 parton correlator 3434 I take from Hayk
      M0M0_3434 = Tqq4(3,4,3,4)*m0_OL
...
C The result for H2:
      ...
      javi_H2qqdelta = M1M1 + M2M0 + (M1M0*(-(Ci*Pi**2.)/3.d0
     &  + 4.d0*CF*h1_TT(3,3,beta,cost)))/
     &  2.d0 + M0M0*((Ci**2.*Pi**4.)/72.d0 + 
     &  (Ci*(CA*(4856.d0 - 603.d0*Pi**2. + 18.d0*Pi**4. - 2772.d0*z3) +
     &  xnf*(-656.d0 + 90.d0*Pi**2. + 504.d0*z3) - 
...
C I add the term for the scale mismatch between mt and M in 2-loop virtuals
      ...
      javi_H2qqdelta = javi_H2qqdelta + 
     &    2.d0*dlog(sn/mt**2.d0)*(b1*M0M0 + b0*M1M0 
     &           - b0**2.d0*M0M0*dlog(sn/mt**2.d0))


c      goto 1234
C I add now the additional term due to the fact that the subtraction
C is performed for mu=mt instead of mu=M
C I start by defining gama1q and gama2q and K

      gama1 = (-3.d0*CF)/4.d0
      gama2 = ((CF*(130.d0/27.d0 + (2.d0*Pi**2.d0)/3.d0)*xnf)/2.d0 + 
     &  CF**2.d0*(-3.d0/2.d0 + 2.d0*Pi**2.d0 - 24.d0*z3) +
     &  CA*CF*(-961.d0/54.d0 - (11.d0*Pi**2.d0)/6.d0 + 26.d0*z3))/16.d0
      KK = (67.d0/18.d0-Pi**2.d0/6.d0)*CA-5.d0/9.d0*xnf
      gamaN = (20.d0*CF*xnf)/9.d0 + CA*CF*(-98.d0/9.d0
     &        + (2.d0*Pi**2.d0)/3.d0 - 4.d0*z3)
...
C TODO CHECK IMAGINARY PART
     ...
     & )
      ...
      javi_H2qqdelta = javi_H2qqdelta + dble(aux1C)
...
C I write now the term 2*Re(<M0|U2-U1^2|M0>)
      ...
      do i = 3,4
      do j = 1,2
        M0M04c(i,j,3,4) = Tqq4(j,i,3,4)*m0_OL
        do k = 3,4
        do l = 1,2
          M0M04c(i,j,k,l) = Tqq4(j,i,l,k)*m0_OL
        enddo
        enddo
      enddo
      enddo
     ...
     & )*M0M04c(i,j,k,l)
      ...
      javi_H2qqdelta = javi_H2qqdelta + dble(aux2C)


c      write(*,*)'aux1 = ',dble(aux1C)
c      write(*,*)'aux2 = ',dble(aux2C)
...
c I add a factor 1/4 because Hayk's expansion is in powers
c of as/pi, while our formula is for as/(2*pi)
      javi_H2qqdelta=javi_H2qqdelta/4.d0

C I normalize by M0M0 directly here
      javi_H2qqdelta=javi_H2qqdelta/M0M0
