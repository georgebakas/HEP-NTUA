...
#{{{ def: get_jobs_in_cluster_queue(self)
    ...
    # this function returns the number of jobs that are currently in the cluster queue
        ...
        try:
            request = subprocess.Popen(self.command_list_queue, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
        except:
            try:
                request = subprocess.Popen(self.command_list_queue, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
            except:
...
#{{{ def: cluster_job_remove(self,job_id)
    ...
    # this routine removes a job that is currently running
        try:
            remove = subprocess.Popen([self.command_kill_job,job_id], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
        except:
            try:
                remove = subprocess.Popen([self.command_kill_job,job_id], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
            except:
...
#{{{ def: terminate_cluster_runs(self)
    def terminate_cluster_runs(self):
        ...
        for job_id in job_ids:
            try:
                terminate = subprocess.Popen([self.command_kill_job,job_id], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
            except:
                try:
                    terminate = subprocess.Popen([self.command_kill_job,job_id], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
                except:
...
#{{{ def: get_nr_of_cluster_jobs(self,status)
    ...
    # with ids from the array job_ids (created inside the routine) with the given status
        ...
        try:
            request_tmp = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
        ...
        if status == "pending":
            nr_of_jobs = request.count(self.job_status_list[0])
        elif status == "running":
...
#Job state, compact form: PD (pending), R (running), CA (cancelled), CF(configuring), CG (completing), CD (completed), F (failed), TO (timeout), NF (node failure) and SE (special exit state).
        self.job_status_list = [" PD "," R "," *not set* "]  # structure: [pending,running,(held)]
#}}}
#{{{ def: get_list_queue_command_with_multiple_ids(self,job_ids)
    def get_list_queue_command_with_multiple_ids(self,job_ids):
    # this routine returns the cluster-specific command needded when listing jobs with a number of job ids
...
#{{{ def: get_job_id_from_content_of_submit_output(self,content)
    ...
    # this routine extracts the job ID from content, which is the saved on-screen output of the job submit on the cluster
        ...
        if not job_id:
            out.print_warning("Could not read job id from fourth word in file "+batch_file_name+".")
            out.print_warning("Resubmitting job in 15 seconds...")
            time.sleep(15)
            return "resubmit"
        return job_id
        ...
        for in_line in content:
            ...
            if "job(s) submitted to cluster" in line:
                try:
                    ...
                    job_id = ""
        if not job_id:
            out.print_warning("Could not read job id from sixth word in second line of file "+batch_file_name+".")
            out.print_warning("Resubmitting job in 15 seconds...")
            time.sleep(15)
            return "resubmit"
        return job_id
...
#        self.job_status_list = [" I "," R "," H "]  # structure: [pending,running,(held)]
        ...
        self.job_status_list = ["1","2","5"]  # structure: [pending,running,(held)]
#}}}
#{{{ def: get_list_queue_command_with_multiple_ids(self,job_ids)
    def get_list_queue_command_with_multiple_ids(self,job_ids):
    # this routine returns the cluster-specific command needded when listing jobs with a number of job ids
...
#{{{ def: __init__(self,config_list,verbose)
    def __init__(self,config_list,verbose):
        ...
        self.job_status_list = [" PEND "," RUN "," *not set* "]  # structure: [pending,running,(held)]
#}}}
#{{{ def: get_list_queue_command_with_multiple_ids(self,job_ids)
    def get_list_queue_command_with_multiple_ids(self,job_ids):
    # this routine returns the cluster-specific command needded when listing jobs with a number of job ids
        ...
        for in_line in content:
            ...
            if "is submitted to" in line:
                try:
                    ...
                    job_id = ""
        if not job_id:
            out.print_warning("Could not read job id within \"<\" and \">\" in file "+batch_file_name+".")
            out.print_warning("Resubmitting job in 15 seconds...")
            time.sleep(15)
            return "resubmit"
        return job_id
...
#{{{ def: __init__(self,config_list,verbose)
    def __init__(self,config_list,verbose):
        ...
        self.job_status_list = [" Q "," R "," *not set* "]  # structure: [pending,running,(held)]
#}}}
#{{{ def: get_list_queue_command_with_multiple_ids(self,job_ids)
    def get_list_queue_command_with_multiple_ids(self,job_ids):
    # this routine returns the cluster-specific command needded when listing jobs with a number of job ids
...
#{{{ def: submit_to_cluster(self,batch_file_name)
    ...
    # this routine executes the cluster-specific submit to the cluster
        with open(batch_file_name,'r') as batch_file:
          with open(batch_file_name+".out",'w') as batch_file_out:
            submit = subprocess.Popen(["pbs"],stdin=batch_file,stdout=batch_file_out)
        return submit
#}}}
#{{{ def: get_job_id_from_content_of_submit_output(self,content)
    def get_job_id_from_content_of_submit_output(self,content):
...
#{{{ def: __init__(self,config_list,verbose)
    def __init__(self,config_list,verbose):
        ...
        self.job_status_list = [" qw "," r "," *not set* "]  # structure: [pending,running,(held)]
#}}}
#{{{ def: get_list_queue_command_with_multiple_ids(self,job_ids)
    def get_list_queue_command_with_multiple_ids(self,job_ids):
    # this routine returns the cluster-specific command needded when listing jobs with a number of job ids
...
get_cluster_class_from_name = {'condor': condor_cluster,'condor_lxplus': condor_lxplus_cluster, 'LSF': lsf_cluster, 'slurm': slurm_cluster, 'PBS': pbs_cluster, 'Torque': pbs_cluster, 'SGE': sge_cluster}
