...
void deepAK8_efficienciesOnlyRecoCutsBkg(float deepAK8Cut = 0.6, float selMvaCut = 0.2, float mvaCut = 0.8)
{
  ...
  if(!outFile->GetListOfKeys()->Contains(TString::Format("Bkg_Reco_tTagger_%.1f", selMvaCut)))
  {
    efficiency_reco[0]->Write(TString::Format("Bkg_Reco_tTagger_%.1f", selMvaCut));
  }
  
  if(!outFile->GetListOfKeys()->Contains(TString::Format("Bkg_Reco_eventMVA_%.1f", mvaCut)))
  {
    efficiency_reco[2]->Write(TString::Format("Bkg_Reco_eventMVA_%.1f", mvaCut));
  }
  
  if(!outFile->GetListOfKeys()->Contains(TString::Format("Bkg_Reco_deepAK8_%.1f", deepAK8Cut)))
  {
    efficiency_reco[1]->Write(TString::Format("Bkg_Reco_deepAK8_%.1f", deepAK8Cut));
  }
