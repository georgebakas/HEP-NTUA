...
void deepAK8_efficienciesJetPtOnlyRecoCutsBkg(float deepAK8Cut = 0.6, float selMvaCut = 0.2, float mvaCut = 0.8)
{
  BoostedAnalysisInitializer utils("2018", false);
  ...
  int deepAK8events = 0;
  int tTaggerEvents = 0;
  std::cout<<utils.listOfFiles.size()<<std::endl;
  ...
  */
  ...
  std::cout<<"Test"<<std::endl;
  if(!outFile->GetListOfKeys()->Contains(TString::Format("Bkg_Reco_jetPt_tTagger_%.1f", selMvaCut)))
  {
    efficiency_reco[0]->Write(TString::Format("Bkg_Reco_jetPt_tTagger_%.1f", selMvaCut));
  }
  std::cout<<"Test"<<std::endl;
  if(!outFile->GetListOfKeys()->Contains(TString::Format("Bkg_Reco_jetPt_eventMVA_%.1f", mvaCut)))
  {
    efficiency_reco[2]->Write(TString::Format("Bkg_Reco_jetPt_eventMVA_%.1f", mvaCut));
  }
  std::cout<<"Test"<<std::endl;
  if(!outFile->GetListOfKeys()->Contains(TString::Format("Bkg_Reco_jetPt_deepAK8_%.1f", deepAK8Cut)))
  {
    efficiency_reco[1]->Write(TString::Format("Bkg_Reco_jetPt_deepAK8_%.1f", deepAK8Cut));
  }
  std::cout<<"Test"<<std::endl;
}