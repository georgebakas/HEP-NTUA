...
void deepAK8_efficienciesJetPtOnlyRecoCuts(float deepAK8Cut = 0.6, float tTaggerCut = 0.2, float mvaCut = 0.8)
{
  ...
  {
    h_out_reco[0][f] = new TH1F(TString::Format("%s_%s_reco_jetPt", histoNames[f].Data(), "tTagger"), TString::Format("%s_%s_reco_jetPt", histoNames[f].Data(), "tTagger"), N_PT, BND_PT);
    h_out_reco[1][f] = new TH1F(TString::Format("%s_%s_reco_jetPt", histoNames[f].Data(), "deepAK8"), TString::Format("%s_%s_reco_jetPt", histoNames[f].Data(), "deepAK8"), N_PT, BND_PT);
    h_out_reco[2][f] = new TH1F(TString::Format("%s_%s_reco_jetPt", histoNames[f].Data(), "eventMVA"), TString::Format("%s_%s_reco_jetPt", histoNames[f].Data(), "eventMVA"), N_PT, BND_PT);
    
    h_denominator_reco[f] = new TH1F(TString::Format("denom_%s_reco_jetPt", histoNames[f].Data()), TString::Format("denom_%s_reco_jetPt", histoNames[f].Data()), N_PT, BND_PT);
    ...
    for(int iev=0;iev<NN;iev++) 
    {
      ...
      if(nJets > 1 && fabs((*jetEta)[0]) < 2.4 && fabs((*jetEta)[1] <2.4) && (*jetPt)[0] > 400 && (*jetPt)[1] > 400 && mJJ > 1000 && nLeptons == 0/*&& (*triggerBit)[2]*/ )
	    {
        h_denominator_reco[f]->Fill((*jetPt)[0]);
        h_denominator_reco[f]->Fill((*jetPt)[1]);
        //1. category where both are fully top tagged
	        ...
	        if((*jetTtag)[0] > tTaggerCut && (*jetTtag)[1] > tTaggerCut && ((*jetBtagSub0)[0] > floatBTag || (*jetBtagSub1)[0] > floatBTag)
		      ...
		      {
            h_out_reco[0][f]->Fill((*jetPt)[0]);
            h_out_reco[0][f]->Fill((*jetPt)[1]);
		      }
          
          if((*deepAK8)[0] > deepAK8Cut && (*deepAK8)[1] > deepAK8Cut &&  ((*jetBtagSub0)[0] > floatBTag || (*jetBtagSub1)[0] > floatBTag)
          && ((*jetBtagSub0)[1] > floatBTag || (*jetBtagSub1)[1] > floatBTag))
          {
            h_out_reco[1][f]->Fill((*jetPt)[0]);
            h_out_reco[1][f]->Fill((*jetPt)[1]);
          }
          
          if(mva > mvaCut && ((*jetBtagSub0)[0] > floatBTag || (*jetBtagSub1)[0] > floatBTag)
          && ((*jetBtagSub0)[1] > floatBTag || (*jetBtagSub1)[1] > floatBTag))
          {
            h_out_reco[2][f]->Fill((*jetPt)[0]);
            h_out_reco[2][f]->Fill((*jetPt)[1]);
          }
        }
      }
    }
