void sigOverBkg()
{
    TFile *signalFile = TFile::Open("2018/rootFiles/deepAK8_efficienciesOnlyRecoCuts.root");
    ...
    TFile *outFile = TFile::Open("signal_over_background.root", "UPDATE");
    ...
    for(std::vector<float>::iterator workingPoint=workingPoints.begin(); workingPoint!=workingPoints.end(); ++workingPoint)
    {
      ...
      TEfficiency* eff_bkg_deepAK8 = (TEfficiency*) bkgFile->Get(TString::Format("Bkg_Reco_deepAK8_%.1f", *workingPoint));
    ...
    TEfficiency* eff_sig_tTagger = (TEfficiency*) signalFile->Get("Sig_Reco_tTagger_0.2");
    TEfficiency* eff_bkg_tTagger = (TEfficiency*) bkgFile->Get("Bkg_Reco_tTagger_0.2");
    ...
    TEfficiency* eff_bkg_mva = (TEfficiency*) bkgFile->Get("Bkg_Reco_eventMVA_0.8");
