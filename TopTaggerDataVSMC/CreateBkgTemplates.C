...
void CreateBkgTemplates(TString year, TString CUT = "")
{
  ...
  TH1F *hData = (TH1F*)infData->Get("hWt_mTop_0btag_expYield_leading");
  ...
  TFile *infTTMC = TFile::Open(TString::Format("%s/TopTaggerHisto_TT_NominalMC_100.root",year.Data()));  //nominal
  //TFile *infTTMC = TFile::Open(TString::Format("%s/Histo_TT_Mtt-700toInf_100.root",year.Data())); //mtt
  TH1F *hCR_MC = (TH1F*)infTTMC->Get("hWt_mTop_0btag_expYield_leading");

  TFile *infBkg_temp = TFile::Open(TString::Format("%s/TopTaggerHisto_SubdominantBkgs_100.root",year.Data()));
  TH1F *hCR_MCSubdominant = (TH1F*)infBkg_temp->Get("hWt_mTop_0btag_expYield_leading");
  ...
  */
  ...
  for(int icat=0;icat<3;icat++) {
    if (icat==1) continue;
    infBkg = TFile::Open(TString::Format("%s/TopTaggerHisto_SubdominantBkgs_100.root",year.Data()));
    ...
    TH1F *hBkg = (TH1F*)infBkg->Get("hWt_"+VAR+TAG+"_expYield_leading");
