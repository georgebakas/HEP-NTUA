...
void GetHistograms_PS_PDF(TString file_name, TString ttbar_process, TString year = "2016", TString weightType="PSWeights")
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  for(int iweight=0; iweight<weightsSize; iweight++ )
  {
    ...
    outFile[iweight] = TFile::Open(TString::Format("%s/%s/Histograms_%s_%s.root", year.Data(), weightType.Data() ,ttbar_process.Data(), weightName.Data()), "RECREATE");
    outFile[iweight]->cd();
      //write them to file
      for(int ivar = 0; ivar<NVAR; ivar++)
      {
      	hReco[iweight][ivar]->Write();
        hParticle[iweight][ivar]->Write();

        hRecoParticle[iweight][ivar]->Write();
        hParticleReco[iweight][ivar]->Write();

        hRecoParton[iweight][ivar]->Write();
        hPartonReco[iweight][ivar]->Write();

        hParton[iweight][ivar]->Write();
      }//end of ivar
