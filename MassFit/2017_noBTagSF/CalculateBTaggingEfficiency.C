...
void initFiles()
{
	...
	llistOfFiles.push_back("TT_Mtt-700to1000_TuneCP5_13TeV-powheg-pythia8.root");
...
void CalculateBTaggingEfficiency()
{
  ...
  const int N_ETA = 3;
  const int N_PT = 8;
  float BND_ETA[N_ETA+1]= {0.0, 1.5, 2,2.4 };
  float BND_PT[N_PT+1] = {0,100,200,300,400,500,600,800,1500}; 
 ...
 for (int f =0; f<listOfFiles.size(); f++)
 {
	...
	for(int ihist = 0; ihist<4; ihist++)
	{
		hBTagParton[f][ihist] = new TH2F(TString::Format("hTagParton_%s_%s", fileHistoNames[f].Data(),histoNames[ihist].Data()), 
			TString::Format("hTagParton_%s_%s", fileHistoNames[f].Data(),histoNames[ihist].Data()), N_PT,BND_PT,N_ETA,BND_ETA);
    hBTagRecoParton[f][ihist] = new TH2F(TString::Format("hTagRecoParton_%s_%s", fileHistoNames[f].Data(),histoNames[ihist].Data()), 
      TString::Format("hTagRecoParton_%s_%s", fileHistoNames[f].Data(),histoNames[ihist].Data()), N_PT,BND_PT,N_ETA,BND_ETA );
	}

  hBTagReco[f] = new TH2F(TString::Format("hBTagReco_%s", fileHistoNames[f].Data()), 
			TString::Format("hBTagReco_%s", fileHistoNames[f].Data()), N_PT,BND_PT, N_ETA, BND_ETA);
 ...
 */
 ...
 TFile *outf = new TFile("BTaggingEfficiency_massCutLoose_BND.root", "RECREATE");
 outf->cd();
