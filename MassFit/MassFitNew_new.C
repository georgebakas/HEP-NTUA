...
void MassFitNew(TString year = "2016", TString ALIAS="",TString CUT="", int REBIN= 5)
{
  ...
  //h2b->Rebin(2);
  // -----------------------------------------
  TFile *fTemplatesBkg = TFile::Open(TString::Format("%s/templates_Bkg_100_2.root", year.Data()));
  TFile *fTemplatesSig = TFile::Open(TString::Format("%s/templates_Sig_100_2.root",year.Data()));
	TFile *fTemplatesQCD = TFile::Open(TString::Format("%s/templates_QCD_100.root",year.Data()));
  RooWorkspace *wTemplatesBkg = (RooWorkspace*)fTemplatesBkg->Get("w");
	RooWorkspace *wTemplatesQCD = (RooWorkspace*)fTemplatesQCD->Get("w");
  ...
  RooAbsPdf *pdf_qcd_2b = (RooAbsPdf*)wTemplatesQCD->pdf("qcd_pdf");
