void CreateBkgTemplates_new(TString CUT = "")
{
  gROOT->ForceStyle();
  ...
  */
  ...
  RooRealVar bQCD4("qcd_b4","qcd_b4",0.8,0,1.6);
  RooRealVar bQCD5("qcd_b5","qcd_b5",0.8,0,1.6);
  RooRealVar bQCD6("qcd_b6","qcd_b6",0.8,0,1.6);
  RooRealVar bQCD7("qcd_b7","qcd_b7",0.8,0,1.6);
  RooRealVar bQCD8("qcd_b8","qcd_b8",0.8,0,1.6);
  RooBernstein qcd1("qcd_brn","qcd_brn",*x,RooArgList(bQCD0,bQCD1,bQCD2, bQCD3, bQCD4, bQCD5, bQCD6,bQCD7, bQCD8));
  ...
  RooRealVar fqcd3("qcd_f3","qcd_f3",0.5,0,1);

  RooRealVar mW("meanW", "meanW", 80, 75, 85);
  RooRealVar sW("sigmaW", "sigmaW", 20,10,40);
  RooGaussian qcd3("qcd_W_gaus", "qcd_W_gaus", *x, mW, sW);

  RooAddPdf *qcd = new RooAddPdf("qcd_pdf","qcd_pdf",RooArgList(qcd1,qcd2,qcd3), RooArgList(fqcd1, fqcd2, fqcd3));
  */
  RooHistPdf *histQCDpdf = new RooHistPdf("QCDhist_pdf", "QCDhist_pdf", RooArgList(*x), *roohData);
  //---- plots ---------------------------------------------------
  ...
  //qcd->plotOn(frameQCD,RooFit::Components("qcd_W_gaus"),RooFit::LineColor(kOrange+1),RooFit::LineWidth(2),RooFit::LineStyle(2));
  frameQCD->GetXaxis()->SetTitle("m_{t} (GeV)");
  //list <Double_t> *lst = histQCDpdf->plotSamplingHint(*x, 50,300);
  histQCDpdf->plotOn(frameQCD,RooFit::LineColor(kRed), RooFit::LineWidth(2), RooFit::LineStyle(2));
  frameQCD->Draw();
  gPad->Update();
  ...
  w->writeToFile("templates_Bkg_"+CUT+"100_NewTest.root");
}                            
