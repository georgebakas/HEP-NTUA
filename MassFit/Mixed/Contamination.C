...
void Contamination(TString year, bool isLoose = true)
{
  gStyle->SetOptStat(0);
  TString looseStr = "_Loose";
  TString dirLoose = "Loose";
  if(!isLoose)
  {
      looseStr = "";
      dirLoose = "Medium";
  ...
  //data template
  TFile *infDataLoose = TFile::Open(TString::Format("%s/Histo_Data_%s_100%s.root", year.Data(),year.Data(), looseStr.Data()));
  //qcd template
  TFile *infQCDMCLoose = TFile::Open(TString::Format("%s/Histo_QCD_HT300toInf_100%s.root", year.Data(),looseStr.Data()));
  //ttbar 
  TFile *infTTMCLoose = TFile::Open(TString::Format("%s/Histo_TT_NominalMC_100%s.root",year.Data(),looseStr.Data()));  //nominal
  //TFile *infTTMCLoose = TFile::Open(TString::Format("%s/Histo_TT_Mtt-700toInf_100_Loose.root",year.Data())); //mtt
  //subdominant bkgs
  TFile *infSubBkg = TFile::Open(TString::Format("%s/Histo_SubdominantBkgs_100%s.root",year.Data(),looseStr.Data()));  //nominal
  ...
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
    ...
    hData[ivar] = (TH1F*)infDataLoose->Get(TString::Format("hWt_%s_0btag_expYield",recoVar.Data()));
    hCR_tt[ivar] = (TH1F*)infTTMCLoose->Get(TString::Format("hWt_%s_0btag_expYield",recoVar.Data()));
    hCR_Subdominant[ivar] = (TH1F*)infSubBkg->Get(TString::Format("hWt_%s_0btag_expYield",recoVar.Data()));
    hCR_QCD[ivar] = (TH1F*)infQCDMCLoose->Get(TString::Format("hWt_%s_0btag_expYield",recoVar.Data()));
