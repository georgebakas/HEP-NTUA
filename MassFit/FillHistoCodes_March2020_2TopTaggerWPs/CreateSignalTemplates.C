void CreateSignalTemplates(TString year, TString CUT = "")
{
  ...
  for(int icat=0;icat<3;icat++) {
    ...
    RooAddPdf *signal;
    if(icat ==0) signal = new RooAddPdf("ttbar_pdf_"+CAT,"ttbar_pdf_"+CAT,RooArgList(sigW,sigComb),RooArgList(fsig2));
    else signal = new RooAddPdf("ttbar_pdf_"+CAT,"ttbar_pdf_"+CAT,RooArgList(sigTop,sigW,sigComb,g1),RooArgList(fsig1,fsig2,fsig3));
