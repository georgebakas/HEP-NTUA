  ...
  for(int ivar =0; ivar< NVAR; ivar++)
  {
  	for(int ireg=0; ireg<controlRegions; ireg++)
  	{
	    ...
	    if(ivar < 7)
	    {
	      ...
	      hCR[f][ireg][ivar] = new TH1F(TString::Format("hCR_%s_%s_%s",histoNames[f].Data(),varReco[ivar].Data(), regions[ireg].Data()), TString::Format("hCR_%s_%s_%s",histoNames[f].Data(),varReco[ivar].Data(), regions[ireg].Data()), sizeBins, 50,300);
