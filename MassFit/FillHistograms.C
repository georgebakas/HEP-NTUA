...
void FillHistograms(int sel = 0)
{
 ...
 for(int f=0; f<listOfFiles.size(); f++)
 {
  ...
  TFile *outFile;
  if(selection ==0)
    outFile = new TFile("Histo_JetHT_Run2016-17Jul2018_New_100.root", "UPDATE");
  if(selection ==1)
    outFile = new TFile("Histo_TT_Mtt-700toInf_TuneCUETP8M2T4_13TeV-powheg-pythia8_New_100.root", "UPDATE");
  if(selection ==2)
    outFile = new TFile("Histo_QCD_HT300toInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_New_100.root", "UPDATE");
  if(selection ==3)
