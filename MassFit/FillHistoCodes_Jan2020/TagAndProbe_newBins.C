...
void TagAndProbe(TString y="2016_preVFP", int sel = 0)
{
 ...
 for(int f=0; f<listOfFiles.size(); f++)
 {
  ...
  TFile *outFile;
  if(selection == 0)
    outFile = new TFile(TString::Format("../%s/TagAndProbeHisto_Data_newBins.root",year.Data()), "RECREATE");
  else if(selection == 2)
    outFile = new TFile(TString::Format("../%s/TagAndProbeHisto_QCD_HT300toInf_newBins.root",year.Data()), "RECREATE");
  else if(selection == 3)
    outFile = new TFile(TString::Format("../%s/TagAndProbeHisto_SubdominantBkgs_newBins.root",year.Data()), "RECREATE");
  else if(selection == 4)
    outFile = new TFile(TString::Format("../%s/TagAndProbeHisto_1000_TT_Nominal_newBins.root",year.Data()), "RECREATE");
