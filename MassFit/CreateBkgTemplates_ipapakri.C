...
void CreateBkgTemplates_ipapakri(TString year, TString CUT = "")
{
  bool extended = true;
 ...
 // AnalysisConstants::initConstants();
  ...
  w->import(*x);
  /*
  TFile *infData = TFile::Open(AnalysisConstants::files[year][TString::Format("%s%s", (extended ? "dataExtended" : "data"), fileExtension.Data())]);
  TFile *infBkg_temp = TFile::Open(AnalysisConstants::files[year][TString::Format("%s%s", (extended ? "mcSubExtended" : "mcSub"), fileExtension.Data())]);
  TFile *infTTMC = TFile::Open(AnalysisConstants::files[year][TString::Format("%s%s", (extended ? "mcSigExtendedNominal" : "mcSigNominal"), fileExtension.Data())]);

  TH1F *hData = MergeHistograms<TH1F>(infData, year, "0btag_leadingJetMassSoftDrop_JetHT", AnalysisConstants::luminositiesCR[year]);
  TH1F *hCR_MCSubdominant = MergeHistograms<TH1F>(infBkg_temp, year, "0btag_leadingJetMassSoftDrop", AnalysisConstants::luminositiesCR[year]);
  TH1F *hCR_MC = MergeHistograms<TH1F>(infTTMC, year, "0btag_leadingJetMassSoftDrop", AnalysisConstants::luminositiesCR[year]);
  */

  TFile *dataFile = TFile::Open(TString::Format("%s/Histo_Data_%s_100.root", year.Data(), year.Data()));
  TH1F *hData = (TH1F *)dataFile->Get("hWt_mTop_0btag_expYield");
  ...
  TFile *sigFile = TFile::Open(TString::Format("%s/Histo_TT_NominalMC_100.root", year.Data())); //nominal
  //TFile *infTTMC = TFile::Open(TString::Format("%s/Histo_TT_Mtt-700toInf_100.root",year.Data())); //mtt
  TH1F *hCR_MC = (TH1F *)sigFile->Get("hWt_mTop_0btag_expYield");

  TFile *subFile = TFile::Open(TString::Format("%s/Histo_SubdominantBkgs_100.root", year.Data()));
  TH1F *hCR_MCSubdominant = (TH1F *)subFile->Get("hWt_mTop_0btag_expYield");
  ...
  */
  ...
  for (int icat = 2; icat < 3; icat++)
  {
    ...
    {
      hBkg = (TH1F *)subFile->Get("hWt_mTop_2btag_expYield");
    }
    hBkg->Rebin(2);
