...
void PlotCutFlow(TString year, TString infStr, TString histo_names)
{
  cout<<year<<endl;
  cout<<infStr<<endl;
  ...
  TString cuts[] = {"Baseline", "jetPt", "mJJ", "jetMassSoftDrop", "btag", "topTagger"};
  ...
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
    ...
    for(int icut=0; icut<NCUTS; icut++)
    {
      ...
      if(!vars[ivar].EqualTo("mJJ"))hCut[ivar][icut]->GetXaxis()->SetTitle(vars[ivar]);
      else hCut[ivar][icut]->GetXaxis()->SetTitle(vars[ivar] + " (GeV)");
    ...
    can[ivar]->Print(TString::Format("%s/CutFlowPlots/CutFlow_%s_%s.pdf", year.Data(), vars[ivar].Data(), histo_names.Data()), "pdf");
  }
