...
# EXECUTE datacards
def executeDataCards(mass_cut):

    for key, value in mass_cut.items():
        file_name = "datacard_chi_SR_"+key+"_cut_"+str(value)+".txt"
        combine_command = "combine -M AsymptoticLimits -d "+file_name+" -n ."+key+"Cut"+str(value)
        print ""
...
def getLimits_wrt(year, file_name):
    print year+'/'+file_name
    if 'combined' not in year:
        file = TFile('../'+year+'_test/'+file_name)
    else:
        file = TFile(file_name)
    tree = file.Get("limit")
...
def ploltUpperLimits_wrt(file_names, values, width, mass_cut, year):
    year_wrt = '2018'
    if width == 1:
        #here you have 1200_12, 1400_14, 1600_16, 1800_18, 2000_20,
        #              2500_25, 3000_30, 3500_35, 4000_40, 4500_45
        if '2016' in year: 
            cross_section = [1.741e+00, 9.097e-01, 4.990e-01, 2.839e-01, 1.655e-01,
                            4.667e-02, 1.496e-02, 5.119e-03, 1.902e-03, 7.609e-04]
            cross_section_wrt = [1.736e+00, 9.096e-0, 5.742e-01, 2.830e-01, 1.662e-01,
                        4.749e-02, 1.494e-02, 5.105e-03, 1.900e-03, 7.613e-04]
                        
        elif '2017' in year:
            cross_section = [1.730e+00, 9.095e-01, 5.002e-01, 2.833e-01, 1.662e-01,
                            4.749e-02, 1.494e-02, 5.105e-03, 1.900e-03, 7.613e-04]
            cross_section_wrt = [1.736e+00, 9.096e-01, 5.742e-01, 2.830e-01, 1.662e-01,
                        4.749e-02, 1.494e-02, 5.105e-03, 1.900e-03, 7.613e-04]
    ...
    for i in range(len(file_names)):
        ...
        if ('2016' in year) and (i == 1): # because 1400_14 doesn't exist for 2016
            continue
        print i, cross_section_wrt[i], cross_section[i]
        limit = getLimits_wrt(year, file_name)
        limit_denom = getLimits_wrt(year_wrt, file_name)
        limit = [x * cross_section[i] for x in limit]
        limit_denom = [x * cross_section_wrt[i] for x in limit_denom]
        #print 'cross_section:', cross_section[i]
        for ilim in range(6):
            print i, values[i], limit[5], limit_denom[5]

        up2s.append(limit[4]/limit_denom[4])
        observed.SetPoint(  i,    values[i], limit[5]/limit_denom[5] ) # observed
        yellow.SetPoint(    i,    values[i], limit[4]/limit_denom[4] ) # + 2 sigma
        green.SetPoint(     i,    values[i], limit[3]/limit_denom[3] ) # + 1 sigma
        median.SetPoint(    i,    values[i], limit[2]/limit_denom[2] ) # median
        green.SetPoint(  2*N-1-i, values[i], limit[1]/limit_denom[1] ) # - 1 sigma
        yellow.SetPoint( 2*N-1-i, values[i], limit[0]/limit_denom[0] ) # - 2 sigma
...
#    legend.AddEntry(green, "Asymptotic CL_{s} #pm 2 std. deviation",'f')
    ...
    label = 'UpperLimit_w{}_slidingMassCut_{}_wrt{}.png'.format(width, year, year_wrt)
    c.SaveAs(label)
    c.Close()
...
#    legend.AddEntry(green, "Asymptotic CL_{s} #pm 2 std. deviation",'f')
    ...
    label = 'UpperLimit_w{}_slidingMassCut_combined.png'.format(width)
    c.SaveAs(label)
    c.Close()
...
def main(year, width):
    ...
    # these are the values for the Z' masses
    ...
    #executeDataCards(mass_cut)
    #plotUpperLimits(file_names, values, width, mass_cut)
    ploltUpperLimits_wrt(file_names, values, width, mass_cut, year)
