...
void ComposeHistograms_test(TString year="2017", int mJJCut = 1000)
{
  ...
  //our new SR is: SR (old) + mJJ > mJJCut
  TH1F *hTT = (TH1F*)infTTfile->Get("hWt_chi_2btag_expYield");
  ...
  //output files
  TFile *outf_data = new TFile(TString::Format("%s/Datacard_test/DataFile_%d.root",year.Data(), mJJCut), "RECREATE");
  outf_data->cd();
  hData->Write("h_Data");

  TFile *outf_processes = new TFile(TString::Format("%s/Datacard_test/ProcessesFile_%d.root", year.Data(),mJJCut), "RECREATE");
  outf_processes->cd();
  ...
  for (int imass = 0; imass<number_of_masses; imass++)
  {
    ...
    for(int iw = 0; iw<1; iw++)
    {
      ...
      outf_Zprime = new TFile(TString::Format("%s/Datacard_test/ZprimeFile_%d_%d_massCut%d.root",
