...
void FillHistograms_Extended_nominal(TString file_name, TString ttbar_process, TString year = "2016")
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  TFile *outFile = TFile::Open(TString::Format("%s/Nominal/Histo_%s.root", year.Data(), ttbar_process.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
