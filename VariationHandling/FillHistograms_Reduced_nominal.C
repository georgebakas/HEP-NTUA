...
void FillHistograms_Reduced_nominal(TString file_name, TString ttbar_process, TString year = "2016")
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  std::vector<float> *y_ = new std::vector<float>(0);
    ...
    TFile *outFile;
    outFile = TFile::Open(TString::Format("%s/Nominal/HistoReduced_%s.root", year.Data(), ttbar_process.Data()), "RECREATE");
    //outFile->cd();
    //write them to file
    for(int ivar = 0; ivar<NVAR; ivar++)
    {
