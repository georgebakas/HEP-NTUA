...
void DoCombineAcceptance(TFile *outFile)
{
  ...
  for (unsigned int var = 0; var < AnalysisConstants::unfoldingVariables.size(); var++)
  {
    ...
    for (unsigned int y = 0; y < AnalysisConstants::years.size(); y++)
    {
      ...
      for (unsigned int i = 0; i < AnalysisConstants::variations.size(); i++)
      {
        ...
        denominator = MergeHistograms<TH1F>(file,
                                            ...
                                            TString::Format("AcceptanceDenom_%s", variable.Data()),
                                            AnalysisConstants::luminositiesSR[AnalysisConstants::years[y]]);
        f = new TH1F("Acceptance", "Acceptance", numerator->GetNbinsX(), bins);
        numerator->Sumw2();
        denominator->Sumw2();
        f->Divide(numerator, denominator, 1., 1., "B");
    ...
    TH1F *resultsHisto = new TH1F(TString::Format("Acceptance_%s",
                                                  variable.Data()),
                                  TString::Format("Acceptance_%s",
                                                  variable.Data()),
...
void CombineAcceptance(TFile *outFile)
{
  DoCombineAcceptance(outFile);
}