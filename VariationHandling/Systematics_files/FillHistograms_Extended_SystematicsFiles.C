...
void FillHistograms_Extended_SystematicsFiles(TString file_name, TString ttbar_process, TString year = "2016", float mJJCut = 1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  TFile *outFile;
  outFile = TFile::Open(TString::Format("%s/SystematicsFiles/Histo_%d_%s", year.Data(),(int)mJJCut,file_name.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
