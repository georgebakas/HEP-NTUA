/*
    ...
    This code will address the fact what will the difference be if we use fully correlated betwenn D(x), Ryield and NQCD
*/
...
void SignalExtractionSpecific(TString year = "2016", TString variable = "jetPt0", TString fitRecoVar = "leadingJetPt")
{
    ...
    cout<<"---"<<endl; */
    cout<<"THIS IS PER BIN"<<endl;
    int nbins = hQ_rebinned->GetNbinsX();
    TGraph *gCorr = new TGraph(nbins);
    TGraph *gAntiCorr = new TGraph(nbins);
    
    for(int i =0; i<hQ_rebinned->GetNbinsX(); i++)
    {
        // this is data 
        float data = hSignal->GetBinContent(i+1);
        float dataError = hSignal->GetBinError(i+1);
        ...
        // this is new stuff 
        float qcd = oldContent * corrected_rYield * NQCD * SF[i];

        // S(x) content
        float newContent = data - qcd; 

        // S(x) error 
        float signalVarianceCorrelated = TMath::Power(dataError,2) + TMath::Power(oldContent*corrected_rYield*NQCD_error,2) +
                            TMath::Power(NQCD*corrected_rYield*oldError,2) + TMath::Power(NQCD*oldContent*corrected_error,2) - 
                            2*oldContent*corrected_rYield*dataError*oldError;

        float signalVarianceUncorrelated = TMath::Power(dataError,2) + TMath::Power(oldContent*corrected_rYield*NQCD_error,2) +
                            TMath::Power(NQCD*corrected_rYield*oldError,2) + TMath::Power(NQCD*oldContent*corrected_error,2);

        float signalVarianceAnticorrelated = TMath::Power(dataError,2) + TMath::Power(oldContent*corrected_rYield*NQCD_error,2) +
                            TMath::Power(NQCD*corrected_rYield*oldError,2) + TMath::Power(NQCD*oldContent*corrected_error,2) +
                            2*oldContent*corrected_rYield*dataError*oldError;

        float newErrorCorrelated   = TMath::Sqrt(signalVarianceCorrelated);
        float newErrorUncorrelated  = TMath::Sqrt(signalVarianceUncorrelated);
        float newErrorAnticorrelated   = TMath::Sqrt(signalVarianceAnticorrelated);
        
        cout<<"bin: "<<i+1<<endl;
        cout<<"oldContent: "<<oldContent<<" ± "<<oldError<<endl;
        cout<<"NQCD: "<<NQCD<<" ± "<<NQCD_error<<endl;
        cout<<"corrected_rYield: "<<corrected_rYield<<" ± "<<corrected_error<<endl;
        cout<<"data : "<<data<<" ± "<<dataError<<endl;
        cout<<"scale factor: "<<SF[i]<<endl;
        cout<<"new Content: "<<newContent<<" with error: "<<newErrorCorrelated<<endl;
        
        cout<<"bin: "<<i+1<<" , newErrorCorrelated = "<<newErrorCorrelated<<" ,  newErrorUncorrelated = "<<newErrorUncorrelated<< ", newErrorAnticorrelated = "<<newErrorAnticorrelated<<endl;
        
        hSignal->SetBinContent(i+1, newContent);
        hSignal->SetBinError(i+1, newErrorCorrelated);

        // tgraph with newErrorCorrelated / newErrorUncorrelated 
        gCorr->SetPoint(i, i, newErrorCorrelated/newErrorUncorrelated);
        // tgraph with newErrorAnticorrelated / newErrorCorrelated
        gAntiCorr->SetPoint(i, i, newErrorAnticorrelated/newErrorUncorrelated);
    }
    //cout<<"-----"<<endl;
    //loop on all content:
    TCanvas *canCorr = new TCanvas("cCorr", "cCorr", 800, 600);
    gCorr->SetTitle("Correlation Analysis");   
    gCorr->GetXaxis()->SetTitle("Bins");    
    gCorr->GetYaxis()->SetTitle("#frac{e_{Fully Corr.}}{e_{Uncorrelated}}");    
    gCorr->Draw("AL*");
    canCorr->Print("Correlated.jpg","jpg");

    TCanvas *canAnti = new TCanvas("cAnti", "cAnti", 800, 600);
    gAntiCorr->SetTitle("Correlation Analysis");
    gAntiCorr->GetXaxis()->SetTitle("Bins");  
    gAntiCorr->GetYaxis()->SetTitle("#frac{e_{Fully AntiCorr.}}{e_{Uncorrelated}}");
    gAntiCorr->Draw("AL*");
    canAnti->Print("Anti-Correlated.jpg","jpg");
