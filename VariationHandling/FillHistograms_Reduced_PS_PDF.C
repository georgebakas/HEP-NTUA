...
void FillHistograms_Reduced_PS_PDF(TString file_name, TString ttbar_process, TString year = "2016", TString weightType="PSWeights")
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  for(int iweight=0; iweight<weightsSize; iweight++ )
  {
    ...
    outFile[iweight] = TFile::Open(TString::Format("%s/%s/HistoReduced_%s_%s.root", year.Data(), weightType.Data() ,ttbar_process.Data(), weightName.Data()), "RECREATE");
    outFile[iweight]->cd();
    //write them to file
    for(int ivar = 0; ivar<NVAR; ivar++)
    {
