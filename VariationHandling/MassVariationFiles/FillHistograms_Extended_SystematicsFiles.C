...
void FillHistograms_Extended_SystematicsFiles(TString file_name, TString ttbar_process, TString sysVariation = "mTop", TString year = "2016", float mJJCut = 1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  TFile *outFile;
  outFile = TFile::Open(TString::Format("Histo_%d_%s_%s.root", (int)mJJCut, ttbar_process.Data(), sysVariation.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
