...
float getTopTaggerEfficiency(float pt, TString year);
...
void ResponseMatrices_nominalTopSF(TString file_name, TString ttbar_process, TString year = "2016", float mJJCut=1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  */
  ...
  TFile *outFile = TFile::Open(TString::Format("%s/ResponsesNominalTopSF/ResponsesEfficiency_%s.root", year.Data(), ttbar_process.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
...
float getTopTaggerEfficiency(float pt, TString year)
{
    ...
    if (pt > 400 && pt < 600) sf = topTaggerSF_error_1[year.Data()][0];
    else if (pt > 600 && pt < 800) sf = topTaggerSF_error_2[year.Data()][0];
    else if (pt > 800) sf = topTaggerSF_error_3[year.Data()][0];
    return sf;
}
