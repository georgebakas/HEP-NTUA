...
void FillHistograms_Extended_JES(TString file_name, TString ttbar_process, TString jes_variation, TString year = "2016", float mJJCut = 1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  //TFile *outFile = TFile::Open(TString::Format("%s/JES/Histo_%d_%s_%s.root", year.Data(), (int)mJJCut, ttbar_process.Data(), jes_variation.Data()), "RECREATE");
  TFile *outFile = TFile::Open(TString::Format("Histo_%d_%s_%s.root", (int)mJJCut, ttbar_process.Data(), jes_variation.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
