...
void ResponseMatrices(TString file_name, TString ttbar_process, TString year = "2016", float mJJCut=1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  TFile *outFile = TFile::Open(TString::Format("%s/ResponsesSystematics/ResponsesEfficiency_%s", year.Data(),file_name.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
