...
void FillHistograms_Reduced_topTagging(TString file_name, TString ttbar_process, TString year = "2016", TString variation = "up", float mJJCut = 1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  std::vector<float> topTagWeights(0);
    ...
    dCSVScoreSub1[1] = (*jetBtagSub1DCSVbb)[1] + (*jetBtagSub1DCSVbbb)[1];
	  ...
	  recoCuts = nJets > 1 && fabs((*jetEta)[0]) < 2.4 && fabs((*jetEta)[1]) <2.4 && (*jetPt)[0] > 450 && (*jetPt)[1] > 400 && mJJ> mJJCut && massCut && nLeptons==0;
    ...
    topTagWeights.push_back(topTag_1); //this is leading mass
    topTagWeights.push_back(topTag_2); //this is subleading mass
  ...
  TFile *outFile = TFile::Open(TString::Format("%s/topTaggingVariation/HistoReduced_%d_%s_%s.root", year.Data(),(int)mJJCut, ttbar_process.Data(), variation.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
