...
void ResponseMatrices_topTagging(TString file_name, TString ttbar_process, TString year = "2016",  TString variation = "up" ,float mJJCut=1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  TH2F *hPartonResponse[NVAR], *hParticleResponse[NVAR];
      ...
      hParticle[ivar] = new TH1F(TString::Format("hParticle_%s", varParticle[ivar].Data()), TString::Format("hParticle_%s", varParticle[ivar].Data()), sizeBinsPartonParticle, tempBNDPartonParticle);
         ...
         hPartonResponse[ivar] = new TH2F(TString::Format("hPartonResponse_%s", varReco[ivar].Data()), TString::Format("hPartonResponse_%s", varReco[ivar].Data()),
          sizeBinsPartonParticle, tempBNDPartonParticle,sizeBins, tempBND);
         hParticleResponse[ivar] = new TH2F(TString::Format("hParticleResponse_%s", varReco[ivar].Data()), TString::Format("hParticleResponse_%s", varReco[ivar].Data()),
          sizeBinsPartonParticle, tempBNDPartonParticle,sizeBins, tempBND);
  ...
  */
  ...
  efficiency_parton[ivar]  = new TEfficiency(*hPartonReco[ivar], *hParton[ivar]);
  ...
  acceptance_parton[ivar]  = new TEfficiency(*hRecoParton[ivar], *hReco[ivar]);
  ...
  efficiency_particle[ivar]  = new TEfficiency(*hParticleReco[ivar], *hParticle[ivar]);
  ...
  acceptance_particle[ivar]  = new TEfficiency(*hRecoParticle[ivar], *hReco[ivar]);
  ...
  TFile *outFile = TFile::Open(TString::Format("%s/ResponsestopTaggingVariation/ResponsesEfficiency_%s_%s.root", year.Data(), ttbar_process.Data(),variation.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
...
{
    float sf = 1;
    if (variation.EqualTo("up")) {
        if (pt > 400 && pt < 600) sf = topTaggerSF_error_1[year.Data()][0] + topTaggerSF_error_1[year.Data()][1];
        else if (pt > 600 && pt < 800) sf = topTaggerSF_error_2[year.Data()][0] + topTaggerSF_error_2[year.Data()][1];
        else if (pt > 800) sf = topTaggerSF_error_3[year.Data()][0] + topTaggerSF_error_3[year.Data()][1];
    } 
    else {
        if (pt > 400 && pt < 600) sf = topTaggerSF_error_1[year.Data()][0] - topTaggerSF_error_1[year.Data()][1];
        else if (pt > 600 && pt < 800) sf = topTaggerSF_error_2[year.Data()][0] - topTaggerSF_error_2[year.Data()][1];
