...
void CombineAcceptance(TString variation="Nominal", TString varParton="Parton")
{ 
  ...
  for (unsigned int var = 0; var < AnalysisConstants::unfoldingVariables.size(); var++)
  {
    ...
    for (unsigned int jvar=0; jvar<variationFiles_Hadronic.size(); jvar++)
    {
      ...
      for (unsigned int y = 0; y < AnalysisConstants::years.size(); y++)
      {
        ...
        TEfficiency *acc_had = (TEfficiency*)inf_had->Get(TString::Format("Acceptance%s_%s",varParton.Data(), variable.Data()));
        TEfficiency *acc_sem = (TEfficiency*)inf_sem->Get(TString::Format("Acceptance%s_%s",varParton.Data(), variable.Data()));
        TEfficiency *acc_dil = (TEfficiency*)inf_dil->Get(TString::Format("Acceptance%s_%s",varParton.Data(), variable.Data()));

        TEfficiency *acceptance = (TEfficiency*)acc_had->Clone();
        *acceptance += (*acc_sem);
        *acceptance += (*acc_dil); 

        //Get numerator and denominator
        TH1F *numerator = (TH1F*)acceptance->GetPassedHistogram();
        numerator->Sumw2();
        TH1F *denominator = (TH1F*)acceptance->GetTotalHistogram();
        denominator->Sumw2();
