...
void FillHistograms_Reduced_SystematicsFiles(TString file_name, TString ttbar_process, TString year = "2016", float mJJCut = 1000 )
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  float norm = ((TH1F*)file->Get("eventCounter/GenEventWeight"))->GetSumOfWeights();
    ...
    TFile *outFile;
    outFile = TFile::Open(TString::Format("%s/SystematicsFiles/HistoReduced_mJJ%d_%s", year.Data(),(int)mJJCut,file_name.Data()), "RECREATE");
    //outFile->cd();
    //write them to file
    for(int ivar = 0; ivar<NVAR; ivar++)
    {
