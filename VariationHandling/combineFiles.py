...
for ifile, file_name in enumerate(glob.iglob('{}/{}/Histo_TTToHadronic*.root'.format(year,weightType,tof), recursive=True)):
#for ifile, file_name in enumerate(glob.iglob('{}/Responses{}/*.root'.format(year,weightType,tof), recursive=True)): #for responses
    ...
    if weightType != 'PSWeights':
        ...
        weight_sufix = dot_split[0]
        print(weight_sufix)
        os.system(f'hadd -f {year}/{weightType}/combined/Histo{tof}_TT_{weightType}_{weight_sufix}.root {year}/{weightType}/Histo{tof}_TTToHadronic_{weight_sufix}.root {year}/{weightType}/Histo{tof}_TTToSemiLeptonic_{weight_sufix}.root {year}/{weightType}/Histo{tof}_TTTo2L2Nu_{weight_sufix}.root')




    #print(f'hadd {year}/{weightType}/combined/Histo{tof}_TT_{weightType}_{weight_sufix}.root {year}/{weightType}/Histo{tof}_TTToHadronic_{weight_sufix}.root {year}/{weightType}/Histo{tof}_TTToSemiLeptonic_{weight_sufix}.root {year}/{weightType}/Histo{tof}_TTTo2L2Nu_{weight_sufix}.root')



    #os.system(f'root -l -b -q \'ResponseMatrices.C(\"{split_file_name[-1]}\",\"{split_file_underscore[0]}\",\"{year}\")\'')
    #break
