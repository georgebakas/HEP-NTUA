...
void FillHistograms_Reduced_bTag(TString file_name, TString ttbar_process, TString year = "2016", TString variation = "up", float mJJCut = 1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  cout<<"norm: "<<norm<<endl;
   ...
   if (nJets>1)
   {
    ...
    TFile *outFile;
    outFile = TFile::Open(TString::Format("%s/bTagVariation/HistoReduced_%d_%s_%s.root", year.Data(), (int)mJJCut,ttbar_process.Data(), variation.Data()), "RECREATE");
    //outFile->cd();
    //write them to file
    for(int ivar = 0; ivar<NVAR; ivar++)
    {
