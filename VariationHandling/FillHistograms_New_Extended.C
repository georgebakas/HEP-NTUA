...
void FillHistograms_New_Extended(TString file_name, TString ttbar_process, TString year = "2016")
{
  ...
  TString varReco[NVAR]   = {"mJJ", "ptJJ", "yJJ","jetPt0","jetPt1", "jetY0", "jetY1","mTop_Leading", "mTop_Subleading", "chi","cosTheta_0", "cosTheta_1"};
  ...
  const int mJJbins = 100;
      ...
      if(ivar ==7 || ivar ==8)
      {
        ...
        hRecoCR[ivar] = new TH1F(TString::Format("hReco_%s", varReco[ivar].Data()), TString::Format("hReco_%s",varReco[ivar].Data()), sizeBins, minJetMass,maxJetMass);
      }
      else
      {
        int sizeBins = NBINS[ivar];
        ...
        hRecoCR[ivar] = new TH1F(TString::Format("hReco_%s", varReco[ivar].Data()), TString::Format("hReco_%s",varReco[ivar].Data()), sizeBins, tempBND);
      }
      hReco[ivar]->Sumw2();
    }
