...
void CreateSignalTemplates_TT(TString year, TString dir, TString inputFile, TString CUT = "")
{
  gROOT->ForceStyle();
  ...
  TFile *inf = TFile::Open(TString::Format("%s/%s/Histo_%s.root", year.Data(), dir.Data(), tempFileName.Data())); //nominal TT, TTJets
  ...
  for(int icat=0;icat<3;icat++) {
    ...
    if(icat ==1) continue;
    TH1F *hMC_yield = (TH1F*)inf->Get(TString::Format("hWt_mTop_Leading_%dbtag", icat));
