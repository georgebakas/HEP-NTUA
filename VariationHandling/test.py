...
for ifile in  all_files[year]:
    ...
    print(ifile)
    #os.system(f'root -l -b -q \'FillHistograms_Extended_nominal.C(\"{eospath+ifile}\",\"{split_file_underscore[0]}\",\"{year}\")\'')
    os.system(f'root -l -b -q \'FillHistograms_Reduced_nominal.C(\"{eospath+ifile}\",\"{split_file_underscore[0]}\",\"{year}\")\'')
