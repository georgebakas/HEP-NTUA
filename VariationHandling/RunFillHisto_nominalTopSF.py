...
for ifile in  all_files[year]:
    ...
    for mjj_cut in mJJCuts:
        ...
        if isExtended == 'True':
                os.system(f'root -l -b -q \'FillHistograms_Extended_nominalTopSF.C(\"{eospath+ifile}\",\"{split_file_underscore[0]}\",\"{year}\",{mjj_cut})\'')
        else:
                os.system(f'root -l -b -q \'FillHistograms_Reduced_nominalTopSF.C(\"{eospath+ifile}\",\"{split_file_underscore[0]}\",\"{year}\",{mjj_cut})\'')