...
void FillHistograms_Extended_nominalTopSF(TString file_name, TString ttbar_process, TString year = "2016", float mJJCut = 1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  TFile *outFile = TFile::Open(TString::Format("%s/NominalTopSF/Histo_%d_%s.root", year.Data(),(int)mJJCut, ttbar_process.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
