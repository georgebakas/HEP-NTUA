...
void ResponseMatrices_PS_PDF(TString file_name, TString ttbar_process, TString year = "2016", TString weightType="PSWeights")
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  for(int ivar =0; ivar<NVAR; ivar++)
  {
    for(int iweight=0; iweight<weightsSize; iweight++)
    {
     ...
     hParton[iweight][ivar] = new TH1F(TString::Format("hParton_%s_%s", varParton[ivar].Data(),  weightName.Data()), TString::Format("hParton_%s_%s", varParton[ivar].Data(),  weightName.Data()), sizeBinsPartonParticle, tempBNDPartonParticle);
    ...
    hParticle[iweight][ivar] = new TH1F(TString::Format("hParticle_%s_%s", varParticle[ivar].Data(),weightName.Data()), TString::Format("hParticle_%s_%s", varParticle[ivar].Data(),weightName.Data()), sizeBinsPartonParticle, tempBNDPartonParticle);
       ...
       hRecoParton[iweight][ivar] = new TH1F(TString::Format("hRecoParton_%s_%s", varReco[ivar].Data(),weightName.Data()), TString::Format("hRecoParton_%s_%s", varReco[ivar].Data(),weightName.Data()), sizeBins, tempBND);
       hPartonReco[iweight][ivar] = new TH1F(TString::Format("hPartonReco_%s_%s", varParton[ivar].Data(),  weightName.Data()), TString::Format("hPartonReco_%s_%s", varParton[ivar].Data(),  weightName.Data()), sizeBinsPartonParticle, tempBNDPartonParticle);

       //numerator for particle efficiency (hParticleReco vs particle) and acceptance (hRecoParticle vs reco)
       hRecoParticle[iweight][ivar] = new TH1F(TString::Format("hRecoParticle_%s_%s", varReco[ivar].Data(),weightName.Data()), TString::Format("hRecoParticle_%s_%s", varReco[ivar].Data(),weightName.Data()), sizeBins, tempBND);
       hParticleReco[iweight][ivar] = new TH1F(TString::Format("hParticleReco_%s_%s", varParticle[ivar].Data(),weightName.Data()), TString::Format("hParticleReco_%s_%s", varParticle[ivar].Data(),weightName.Data()), sizeBinsPartonParticle, tempBNDPartonParticle);

       //response matrices
       //x-axis: parton or particle and y-axis: reco (detector level)
       //x-axis will have fewer bins than y-axis
       hPartonResponse[iweight][ivar] = new TH2F(TString::Format("hPartonResponse_%s_%s", varReco[ivar].Data(),weightName.Data()), TString::Format("hPartonResponse_%s_%s", varReco[ivar].Data(),weightName.Data()),
        sizeBinsPartonParticle, tempBNDPartonParticle,sizeBins, tempBND);
       hParticleResponse[iweight][ivar] = new TH2F(TString::Format("hParticleResponse_%s_%s", varReco[ivar].Data(),weightName.Data()), TString::Format("hParticleResponse_%s_%s", varReco[ivar].Data(),weightName.Data()),
        sizeBinsPartonParticle, tempBNDPartonParticle,sizeBins, tempBND);
    }
  }
  ...
  for(int ivar =0; ivar<NVAR; ivar++)
  {
    for(int iweight=0; iweight<weightsSize; iweight++)
    {
      ...
      hRecoParticle[iweight][ivar]->Scale(weights*LUMI);
      hParticleReco[iweight][ivar]->Scale(weights*LUMI);

      hRecoParton[iweight][ivar]->Scale(weights*LUMI);
      hPartonReco[iweight][ivar]->Scale(weights*LUMI);

      hParton[iweight][ivar]->Scale(weights*LUMI);
    }
  }
  TEfficiency *efficiency_parton[weightsSize][NVAR], *acceptance_parton[weightsSize][NVAR];
  ...
  for(int ivar = 0; ivar< NVAR; ivar++)
  {
    for(int iweight=0; iweight<weightsSize; iweight++)
    {
      ...
      if(weightType.EqualTo("PSWeights")) weightName = ps_weights[iweight];
        ...
        if (ivar ==0 || ivar ==1 || ivar ==3 || ivar ==4) efficiency_parton[iweight][ivar]->SetTitle(TString::Format("EfficiencyParton_%s_%s;%s (GeV);Efficiency Parton",varParton[ivar].Data(),weightName.Data(),varParton[ivar].Data()));
        else efficiency_parton[iweight][ivar]->SetTitle(TString::Format("EfficiencyParton_%s_%s;%s ;Efficiency Parton",varParton[ivar].Data(),weightName.Data(),varParton[ivar].Data()));
        efficiency_parton[iweight][ivar]->SetStatisticOption(TEfficiency::kFNormal);
        efficiency_parton[iweight][ivar]->SetUseWeightedEvents();
        efficiency_parton[iweight][ivar]->SetLineColor(kRed);

        acceptance_parton[iweight][ivar]  = new TEfficiency(*hRecoParton[iweight][ivar], *hReco[iweight][ivar]);
        if (ivar ==0 ||ivar ==1 || ivar ==3 || ivar ==4)  acceptance_parton[iweight][ivar]->SetTitle(TString::Format("AcceptanceParton_%s_%s; %s(GeV);Acceptance Parton",varReco[ivar].Data(),weightName.Data(),varReco[ivar].Data()));
        else acceptance_parton[iweight][ivar]->SetTitle(TString::Format("AcceptanceParton_%s_%s; %s;Acceptance Parton",varReco[ivar].Data(), weightName.Data(),varReco[ivar].Data()));
        acceptance_parton[iweight][ivar]->SetStatisticOption(TEfficiency::kFNormal);
        acceptance_parton[iweight][ivar]->SetUseWeightedEvents();
        acceptance_parton[iweight][ivar]->SetLineColor(kRed);


        cout<<"-------"<<endl;
        cout<<"particle "<<ivar<<endl;
        efficiency_particle[iweight][ivar]  = new TEfficiency(*hParticleReco[iweight][ivar], *hParticle[iweight][ivar]);
        if (ivar ==0 || ivar ==1 || ivar ==3 || ivar ==4) efficiency_particle[iweight][ivar]->SetTitle(TString::Format("EfficiencyParticle_%s_%s;%s (GeV);Efficiency Particle",varParticle[ivar].Data(),weightName.Data(),varParticle[ivar].Data()));
        else efficiency_particle[iweight][ivar]->SetTitle(TString::Format("EfficiencyParticle_%s_%s;%s ;Efficiency Particle",varParticle[ivar].Data(),weightName.Data(),varParticle[ivar].Data()));
        efficiency_particle[iweight][ivar]->SetStatisticOption(TEfficiency::kFNormal);
        efficiency_particle[iweight][ivar]->SetUseWeightedEvents();
        efficiency_particle[iweight][ivar]->SetLineColor(kRed);

        acceptance_particle[iweight][ivar]  = new TEfficiency(*hRecoParticle[iweight][ivar], *hReco[iweight][ivar]);
        if (ivar ==0 ||ivar ==1 || ivar ==3 || ivar ==4)  acceptance_particle[iweight][ivar]->SetTitle(TString::Format("AcceptanceParticle_%s_%s; %s(GeV);Acceptance particle",varReco[ivar].Data(),weightName.Data(),varReco[ivar].Data()));
        else acceptance_particle[iweight][ivar]->SetTitle(TString::Format("AcceptanceParticle_%s_%s; %s;Acceptance particle",varReco[ivar].Data(),weightName.Data(), varReco[ivar].Data()));
        acceptance_particle[iweight][ivar]->SetStatisticOption(TEfficiency::kFNormal);
        acceptance_particle[iweight][ivar]->SetUseWeightedEvents();
        acceptance_particle[iweight][ivar]->SetLineColor(kRed);
  ...
  TH1F *purityParton[weightsSize][NVAR], *stabilityParton[weightsSize][NVAR];
  TH1F *purityParticle[weightsSize][NVAR], *stabilityParticle[weightsSize][NVAR];
  ...
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
    for(int iweight=0; iweight<weightsSize; iweight++ )
    {
      ...
      int sizeBins = NBINS[ivar];
        ...
        purityParton[iweight][ivar] 	  = new TH1F(TString::Format("PurityParton_%s_%s", varReco[ivar].Data(), weightName.Data()),TString::Format("PurityParton_%s_%s", varReco[ivar].Data(), weightName.Data()), sizeBins, tempBND);
        stabilityParton[iweight][ivar] = new TH1F(TString::Format("StabilityParton_%s_%s", varReco[ivar].Data(), weightName.Data()),TString::Format("StabilityParton_%s_%s", varReco[ivar].Data(), weightName.Data()), sizeBins, tempBND);

        purityParticle[iweight][ivar] 	  = new TH1F(TString::Format("PurityParticle_%s_%s", varReco[ivar].Data(), weightName.Data()),TString::Format("PurityParticle_%s_%s", varReco[ivar].Data(), weightName.Data()), sizeBins, tempBND);
        stabilityParticle[iweight][ivar] = new TH1F(TString::Format("StabilityParticle_%s_%s", varReco[ivar].Data(), weightName.Data()),TString::Format("StabilityParticle_%s_%s", varReco[ivar].Data(), weightName.Data()), sizeBins, tempBND);
      ...
      {
          sumOfColsParton[i] = ((TH1D*)hPartonResponse[iweight][ivar]->ProjectionX())->GetBinContent(i);
          sumOfRowsParton[i] = ((TH1D*)hPartonResponse[iweight][ivar]->ProjectionY())->GetBinContent(i);

          sumOfColsParticle[i] = ((TH1D*)hParticleResponse[iweight][ivar]->ProjectionX())->GetBinContent(i);
          sumOfRowsParticle[i] = ((TH1D*)hParticleResponse[iweight][ivar]->ProjectionY())->GetBinContent(i);

          for(int j=1; j<=sizeBins; j++)
          {
            if(i==j)
              {
                  float initContentParton = hPartonResponse[iweight][ivar]->GetBinContent(i,j);
                  purityParton[iweight][ivar]->SetBinContent(i,initContentParton/sumOfColsParton[i]);
                  stabilityParton[iweight][ivar]->SetBinContent(i,initContentParton/sumOfRowsParton[i]);

                  float initContentParticle = hParticleResponse[iweight][ivar]->GetBinContent(i,j);
                  purityParticle[iweight][ivar]->SetBinContent(i,initContentParticle/sumOfColsParticle[i]);
                  stabilityParticle[iweight][ivar]->SetBinContent(i,initContentParticle/sumOfRowsParticle[i]);
              }
          }
      }
    }
  ...
  for(int iweight=0; iweight<weightsSize; iweight++ )
  {
    ...
    outFile[iweight]->cd();
      ...
      for(int ivar = 0; ivar<NVAR; ivar++)
      {
      	efficiency_parton[iweight][ivar]->Write(TString::Format("EfficiencyParton_%s",varParton[ivar].Data()));
      	efficiency_particle[iweight][ivar]->Write(TString::Format("EfficiencyParticle_%s",varParticle[ivar].Data()));
        ...
        stabilityParton[iweight][ivar]->Write(TString::Format("StabilityParton_%s",varReco[ivar].Data()));
        purityParton[iweight][ivar]->Write(TString::Format("PurityParton_%s",varReco[ivar].Data()));

        stabilityParticle[iweight][ivar]->Write(TString::Format("StabilityParticle_%s",varReco[ivar].Data()));
        purityParticle[iweight][ivar]->Write(TString::Format("PurityParticle_%s",varReco[ivar].Data()));
      }//end of ivar
