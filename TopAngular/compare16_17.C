...
void initXsections()
{
  XSEC.push_back(16.74); //this is the TT xsec 2016 
  XSEC.push_back(16.74); //this is the TT xsec 2017 
  XSEC.push_back(16.74); //this is the TT xsec 2018
...
void compare16_17(TString recoVar = "Chi",TString partonVar = "chi", bool isDeepCSV = true)
{
  ...
  hBkg_CRExpYield[0] = (TH1F*)hBkg_CR[0]->Clone(TString::Format("CR_tTagger_%s_expYield",recoVar.Data()));
  hBkg_CRExpYield[1] = (TH1F*)hBkg_CR[1]->Clone(TString::Format("CR_tTagger_%s_expYield",recoVar.Data()));
  hBkg_CRExpYield[2] = (TH1F*)hBkg_CR[2]->Clone(TString::Format("CR_tTagger_%s_expYield",recoVar.Data()));
  ...
  hSig_CR[0] = (TH1F*)signal[0]->Get(TString::Format("h%sCR_TT",recoVar.Data()));  //these are not scaled...
  hSig_CR[0] -> SetLineColor(kBlue);
  hSig_CR[0] -> Scale(weights[0]*LUMI[0]);
  hSig_CR[0] -> SetTitle("TT Contamination tTagger '16");
  hSig_CR[1] = (TH1F*)signal[1]->Get(TString::Format("h%sCR_TT",recoVar.Data()));
  hSig_CR[1] -> SetLineColor(kBlue);
  hSig_CR[1] -> Scale(weights[1]*LUMI[1]);
  hSig_CR[1] -> SetTitle("TT Contamination tTagger '17");
  hSig_CR[2] = (TH1F*)signal[2]->Get(TString::Format("h%sCR_TT",recoVar.Data()));
