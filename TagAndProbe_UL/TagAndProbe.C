...
void TagAndProbe(TString y="2016", int sel = 0, bool isLoose=false)
{
  year =y;
 ...
 cout<<"triggerSRConst[year.Data()]]: "<<triggerSRConst[year.Data()]<<endl;
  ...
  TString varReco[NVAR]   = {"mJJ", "ptJJ", "yJJ","jetPt0","jetPt1", "jetY0", "jetY1",
                "mva", "topTagger1", "mTop", "jetMassSoftDrop", "deepAK8Tagger"};
 ...
 //this h_Denominator: events pass SRB and tightTopTaggerCut for 1 jet and no other cut on other jet
//this is h_Numerator: events pass SRB and tight TopTagger + SR
 ...
 // trIN->SetBranchAddress("jetBtagSub1"    ,&jetBtagSub1);
  ...
  for(int ivar =0; ivar< NVAR; ivar++)
  {
    ...
    if(ivar < 7)
    {
      ...
      h_Numerator[f][ivar] = new TH1F(TString::Format("hSRBTightAndSR%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSRBTightAndSR%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, tempBND);
      h_Denominator[f][ivar] = new TH1F(TString::Format("hSRBTightAndProbe_%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSRBTightAndProbe_%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, tempBND);
    }
    else if (ivar == 9 || ivar == 10)
    {
      h_Numerator[f][ivar] = new TH1F(TString::Format("hSRBTightAndSR%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSRBTightAndSR%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, 120,220);
      h_Denominator[f][ivar] = new TH1F(TString::Format("hSRBTightAndProbe_%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSRBTightAndProbe_%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, 120,220);
    }
    else if (ivar == 7 || ivar == 8)
    {
      h_Numerator[f][ivar] = new TH1F(TString::Format("hSRBTightAndSR%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSRBTightAndSR%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, -1,1);
      h_Denominator[f][ivar] = new TH1F(TString::Format("hSRBTightAndProbe_%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSRBTightAndProbe_%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, -1,1);
    }
  ...
  TH1F *h_Numerator_Clone[listOfFiles.size()][NVAR];
  TH1F *h_Denominator_Clone[listOfFiles.size()][NVAR];
  ...
  for(int ivar= 0; ivar<NVAR; ivar++)
  {
    ...
    {
      h_Numerator_Clone[j][ivar]=(TH1F*)h_Numerator[j][ivar]->Clone(TString::Format("hSRBTightAndSR%s_%s_Clone","tTagger",histoNames[j].Data()));
      h_Denominator_Clone[j][ivar]=(TH1F*)h_Denominator[j][ivar]->Clone(TString::Format("hSRBTightAndProbe_%s_%s_Clone","tTagger",histoNames[j].Data()));
        ...
        if(selection !=0)
        {
         ...
         h_Numerator[j][ivar]->Scale(weights[j]); //this is h_Numerator
        }
    }
    ...
    for(int j=1; j<listOfFiles.size(); j++)
    {
      ...
      h_Numerator_Clone[0][ivar]->Add(h_Numerator_Clone[j][ivar]);
      h_Denominator_Clone[0][ivar]->Add(h_Denominator_Clone[j][ivar]);
    }
  ...
  if(selection ==0)
    outFile = new TFile(TString::Format("%s/TagAndProbeHisto_Data_%s_100_reduced_UnequalBinning%s.root",year.Data(),year.Data(),loose.Data()), "RECREATE");
  else if(selection ==1)
    outFile = new TFile(TString::Format("%s/TagAndProbeHisto_TT_Mtt-700toInf_100_reduced_UnequalBinning%s.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==2)
    outFile = new TFile(TString::Format("%s/TagAndProbeHisto_QCD_HT300toInf_100_reduced_UnequalBinning%s.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==3)
    outFile = new TFile(TString::Format("%s/TagAndProbeHisto_SubdominantBkgs_100_reduced_UnequalBinning%s.root",year.Data(),loose.Data()), "RECREATE");
  else if(selection ==4)
    outFile = new TFile(TString::Format("%s/TagAndProbeHisto_TT_NominalMC_100_reduced_UnequalBinning%s.root",year.Data(),loose.Data()), "RECREATE");
  ...
  {
    h_Numerator[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s (GeV)", varNameReco.Data()));
    h_Denominator[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s (GeV)", varNameReco.Data()));

    h_Numerator_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s (GeV)", varNameReco.Data()));
    h_Denominator_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s (GeV)", varNameReco.Data()));
  }
  else
  {
    h_Denominator[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
    h_Numerator[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
    h_Numerator_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
    h_Denominator_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
  }
  ...
  if(ivar == 8)
  {
    h_Numerator[0][ivar-1]->Add(h_Numerator[0][ivar]);
    h_Denominator[0][ivar-1]->Add(h_Denominator[0][ivar]);
    h_Numerator_Clone[0][ivar-1]->Add(h_Numerator_Clone[0][ivar]);
    h_Denominator_Clone[0][ivar-1]->Add(h_Denominator_Clone[0][ivar]);
  }

   outFile->cd();
   h_Numerator[0][ivar]->Write(TString::Format("hSRBTightAndSR_%s", varNameReco.Data()));
   h_Denominator[0][ivar]->Write(TString::Format("hSRBTightAndProbe_%s", varNameReco.Data()));
   h_Numerator_Clone[0][ivar]->Write(TString::Format("hSRBTightAndSR_%s_expYield", varNameReco.Data()));
   h_Denominator_Clone[0][ivar]->Write(TString::Format("hSRBTightAndProbe_%s_expYield", varNameReco.Data()));
