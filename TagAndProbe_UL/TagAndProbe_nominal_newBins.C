...
void TagAndProbe_nominal_newBins(TString file_name, TString ttbar_process, TString year = "2016", float mJJCut = 1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  int minJetMass = 120;
    ...
    TFile *outFile;
    outFile = TFile::Open(TString::Format("%s/Nominal/TagAndProbeHisto_TT_%d_%s_newBins.root", year.Data(),(int)mJJCut, ttbar_process.Data()), "RECREATE");
