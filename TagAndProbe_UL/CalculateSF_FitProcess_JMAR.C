...
void CalculateSF_FitProcess_JMAR(TString year = "2017")
{
    initFilesMapping();
    ...
    // create a histogram 
    Float_t xbins[4] = {400, 500, 600, 1500};
    int NBINS = 3;
    ...
    // SF central line 
    Double_t plotX_data[3] = {400, 550, 1500};
    ...
    leg->Draw();
    can->Update();
    can->Print(TString::Format("%s/Nominal/plots/SF_perPtRegion_FitProcess_JMAR.pdf",year.Data()),"pdf");

    // save the TF1s
    TFile *fit_file = new TFile(TString::Format("../VariationHandling/%s/TopTaggerUncertainty_FitValuesJMAR.root",year.Data()),"RECREATE");
    fit_file->cd();
    fup->Write("fup");
    fdown->Write("fdown");
