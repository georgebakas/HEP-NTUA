...
void CalculateSF_newBins(TString year = "2017")
{
  initFilesMapping();
  ...
  FILE *fp;
  TString str = TString::Format("%s/Output_ScaleFactor_%s_newRegions_.txt",year.Data(), year.Data());
  fp = fopen(str.Data(),"w");
  ...
  leg->Draw();
  can->Update();
  can->Print(TString::Format("%s/Nominal/plots/SF_perPtRegion_newRegions.pdf",year.Data()),"pdf");
