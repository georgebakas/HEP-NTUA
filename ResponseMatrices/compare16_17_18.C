					 ...
					 bool isEqual = true, bool isNominal = true)
{
   ...
   for(int iy = 0; iy<sizeof(years)/sizeof(years[0]); iy++)
   {
      ...
      if(recoVar.EqualTo("jetPt0")) cout<<hResponses[iy][i]->Integral()<<endl;
   		...
   		if(iy ==0) canResponse[iy][i]->Print(TString::Format("%s/%s/%s/%sResponseMatrix%s_%s_New.pdf",years[iy].Data(),binning.Data(), recoVar.Data() ,phaseSpace[i].Data(),nominal.Data(),recoVar.Data()),"pdf");
   	}
   	break;
   }
