...
void initGlobals()
{
 ...
 void qcdClosure_allVars_fit(bool isSig = false)
{
  isSignal = isSig;
  initGlobals();	
  gStyle->SetOptStat(0);
  ...
  TF1 *fitLeadingJetPt = (TF1*)fitFile->Get("myfunc");
 ...
 for(int f=0; f<listOfFiles.size(); f++)
 {
  ...
  for(int iev=0;iev<NN;iev++) 
  {
	...
	else //we are in QCD samples
	{
	 ...
	 fitSF = fitLeadingJetPt->Eval((*jetPt)[0]);
	/* if( (*jetPt)[0] > 400)
	 {
	 cout<<"------"<<endl;
	 cout<<"initial Pt: "<<(*jetPt)[0]<<endl;
	 cout<<"sf from fit: "<<fitLeadingJetPt->Eval((*jetPt)[0])<<endl;

	 
	 cout<<"new Pt: "<<fitSF*(*jetPt)[0]<<endl;
	...
	else 
	{
	 ...
	 for(int ivar = 0; ivar <NVAR; ivar ++)
	 {
		 ...
		 if(recoCuts && revertBtag && massCut && tTaggerCut)
			hCR[0][f][ivar]->Fill(xReco,genEvtWeight*fitSF);
		 ...
		 if(recoCuts && revertBtag && massCut && deepAK8Cut)
			hCR[1][f][ivar]->Fill(xReco,genEvtWeight*fitSF);
		 ...
		 if(recoCuts && revertBtag && massCut && oldMva)
			hCR[2][f][ivar]->Fill(xReco,genEvtWeight*fitSF);
		 ...
		 if(recoCuts && massCut && tTaggerCut && btag1)
			h1Btag[0][f][ivar]->Fill(xReco,genEvtWeight*fitSF);
			
		 //1 btag region with deepAK8
		 if(recoCuts && massCut && deepAK8Cut && btag1)
			h1Btag[1][f][ivar]->Fill(xReco,genEvtWeight*fitSF);
			
		 //1 btag region with oldMva
		 if(recoCuts && massCut && oldMva && btag1)
			 h1Btag[2][f][ivar]->Fill(xReco,genEvtWeight*fitSF);
	 }
  ...
  */
  ...
  if(isSignal) 
  {
	  if(isDeepCSV) outFile = new TFile(TString::Format("SignalOutput_AllRegions_%0.2f_deepCSV_afterFit.root", selMvaCut), "UPDATE");
	  else outFile = new TFile(TString::Format("SignalOutput_AllRegions_%0.2f_CSVv2.root", selMvaCut), "UPDATE");
  }
  else 
  {
	  if(isDeepCSV) outFile = new TFile(TString::Format("BkgOutput_AllRegions_%0.2f_deepCSV_afterFit.root",selMvaCut), "UPDATE");
