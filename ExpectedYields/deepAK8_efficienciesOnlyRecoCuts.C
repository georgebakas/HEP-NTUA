...
void deepAK8_efficienciesOnlyRecoCuts(TString inYear = "2017", float tTaggerCut = 0.1,float deepAK8Cut = 0.6, float mvaCut = 0.8)
{
	 TString year = inYear;
  ...
  if(!outFile->GetListOfKeys()->Contains(TString::Format("Sig_Reco_tTagger_%.2f", tTaggerCut)))
  {
    efficiency_reco[0]->Write(TString::Format("Sig_Reco_tTagger_%.2f", tTaggerCut));
  }
