...
void plotTT_QCD_16_other(TString year1 = "2017", bool isNominalMC = false)
{
...
void plotYearVar(TString year1, TString year2 ,TString recoVar = "jetPt0")
{
  ...
  std::vector<TString> histoNamesTT16;
  std::vector<TString> histoNamesQCD;
  histoNamesTT.clear();
  histoNamesTT16.clear();
  histoNamesQCD.clear();
  ...
  if(!globalIsNominalMC)
  {
  	...
  	histoNamesTT16.push_back("Signal_histo_Mtt_700_1000"); 
  	histoNamesTT16.push_back("Signal_histo_Mtt_1000_Inf");
  }
  else 
  {
  	...
  	histoNamesTT16.push_back("Signal_histo_Nominal");
  }
...
void ratioPlot(TH1F *hNum ,TH1F *hDenom, TString recoVar, TString reason)
{
  ...
  c1->Print(TString::Format("./ComparisonScaledIntegral/%s/%s/comparison_mc%s_%s_%s.pdf",temp.Data(),recoVar.Data(),
  							 globalYear1.Data(), globalYear2.Data(), reason.Data()),"pdf");
}