...
void FillHistograms_Reduced_DY(TString y="2016", int sel = 0, int massWindow=1000)
{
  year =y;
 ...
 TH1F *hCR_Lo1[listOfFiles.size()][NVAR], *hCR_Gr1[listOfFiles.size()][NVAR];
 TH1F *hSR_Lo1[listOfFiles.size()][NVAR], *hSR_Gr1[listOfFiles.size()][NVAR];
 ...
 // cout<<"weight: "<<weights[f]<<endl;
  ...
  for(int ivar =0; ivar< NVAR; ivar++)
  {
    ...
    hCR_Lo1[f][ivar] = new TH1F(TString::Format("hCR_Lo1_%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hCR_%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, tempBND);
    hSR_Lo1[f][ivar] = new TH1F(TString::Format("hSR_Lo1_%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSR_%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, tempBND);

    hCR_Gr1[f][ivar] = new TH1F(TString::Format("hCR_%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hCR_Gr1_%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, tempBND);
    hSR_Gr1[f][ivar] = new TH1F(TString::Format("hSR_%s_%s_%s", "tTagger",histoNames[f].Data(),varReco[ivar].Data()), TString::Format("hSR_Gr1_%s_%s_%s","tTagger",histoNames[f].Data(),varReco[ivar].Data()), sizeBins, tempBND);
  }
  //for matching
  ...
  btag1 = btag1DeepCSV;
   ...
   for(int ivar = 0; ivar <xRecoAll.size(); ivar ++)
   {
    ...
    {
      if (delta_y > 1)
        hSR_Gr1[f][ivar]->Fill(xReco,genEvtWeight*bTagEvntWeight);
      else
        hSR_Lo1[f][ivar]->Fill(xReco,genEvtWeight*bTagEvntWeight);
    }
    //Control Region with tTagger
    if(recoCuts && revertBtag && massCut && tTaggerCut && triggerCR)
    {
      if (delta_y > 1)
        hCR_Gr1[f][ivar]->Fill(xReco,genEvtWeight*bTagEvntWeight);
      else
        hCR_Lo1[f][ivar]->Fill(xReco,genEvtWeight*bTagEvntWeight);
    }
  ...
  TH1F *hCR_Lo1_Clone[listOfFiles.size()][NVAR];
  TH1F *hCR_Gr1_Clone[listOfFiles.size()][NVAR];
  TH1F *hSR_Lo1_Clone[listOfFiles.size()][NVAR];
  TH1F *hSR_Gr1_Clone[listOfFiles.size()][NVAR];
  ...
  for(int ivar= 0; ivar<NVAR; ivar++)
  {
    ...
    {
      hCR_Gr1_Clone[j][ivar]=(TH1F*)hCR_Gr1[j][ivar]->Clone(TString::Format("hCR_%s_%s_Clone","tTagger",histoNames[j].Data()));
      hSR_Gr1_Clone[j][ivar]=(TH1F*)hSR_Gr1[j][ivar]->Clone(TString::Format("hSR_%s_%s_Clone","tTagger",histoNames[j].Data()));
      hCR_Lo1_Clone[j][ivar]=(TH1F*)hCR_Lo1[j][ivar]->Clone(TString::Format("hCR_%s_%s_Clone","tTagger",histoNames[j].Data()));
      hSR_Lo1_Clone[j][ivar]=(TH1F*)hSR_Lo1[j][ivar]->Clone(TString::Format("hSR_%s_%s_Clone","tTagger",histoNames[j].Data()));

        if(selection !=0)
        {
         hCR_Gr1_Clone[j][ivar]->Scale(weights[j]*LUMI_CR); //this is 0 btagged (CR)
         hSR_Gr1_Clone[j][ivar]->Scale(weights[j]*LUMI); //this is 2 btagged (SR)

         hCR_Gr1[j][ivar]->Scale(weights[j]); //this is CR
         hSR_Gr1[j][ivar]->Scale(weights[j]); //this is Signal region

         hCR_Lo1_Clone[j][ivar]->Scale(weights[j]*LUMI_CR); //this is 0 btagged (CR)
         hSR_Lo1_Clone[j][ivar]->Scale(weights[j]*LUMI); //this is 2 btagged (SR)

         hCR_Lo1[j][ivar]->Scale(weights[j]); //this is CR
         hSR_Lo1[j][ivar]->Scale(weights[j]); //this is Signal region
        }
    }
    ...
    for(int j=1; j<listOfFiles.size(); j++)
    {
      ...
      hCR_Gr1[0][ivar]->Add(hCR_Gr1[j][ivar]);
      hSR_Gr1[0][ivar]->Add(hSR_Gr1[j][ivar]);
      hCR_Gr1_Clone[0][ivar]->Add(hCR_Gr1_Clone[j][ivar]);
      hSR_Gr1_Clone[0][ivar]->Add(hSR_Gr1_Clone[j][ivar]);

      hCR_Lo1[0][ivar]->Add(hCR_Lo1[j][ivar]);
      hSR_Lo1[0][ivar]->Add(hSR_Lo1[j][ivar]);
      hCR_Lo1_Clone[0][ivar]->Add(hCR_Lo1_Clone[j][ivar]);
      hSR_Lo1_Clone[0][ivar]->Add(hSR_Lo1_Clone[j][ivar]);
    }
  ...
  TFile *outFile;
  if(selection ==0)
    outFile = new TFile(TString::Format("%s/HistoDeltaY_Data_%s_reduced_%d.root",year.Data(),year.Data(),massWindow), "RECREATE");
  else if(selection ==1)
    outFile = new TFile(TString::Format("%s/HistoDeltaY_TT_Mtt-700toInf_reduced_%d.root",year.Data(),massWindow), "RECREATE");
  else if(selection ==2)
    outFile = new TFile(TString::Format("%s/HistoDeltaY_QCD_HT300toInf_reduced_%d.root",year.Data(),massWindow), "RECREATE");
  else if(selection ==3)
    outFile = new TFile(TString::Format("%s/HistoDeltaY_SubdominantBkgs_reduced_%d.root",year.Data(),massWindow), "RECREATE");
  else if(selection ==4)
    outFile = new TFile(TString::Format("%s/HistoDeltaY_TT_NominalMC_reduced_%d.root",year.Data(), massWindow), "RECREATE");
  ...
  hCR_Gr1[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
  hSR_Gr1[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
  hCR_Gr1_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
  hSR_Gr1_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));

  hCR_Lo1[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
  hSR_Lo1[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
  hCR_Lo1_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));
  hSR_Lo1_Clone[0][ivar]->GetXaxis()->SetTitle(TString::Format("%s", varNameReco.Data()));


  outFile->cd();
  hSR_Gr1[0][ivar]->Write(TString::Format("hWt_Gr1_%s_2btag", varNameReco.Data()));
  hCR_Gr1[0][ivar]->Write(TString::Format("hWt_Gr1_%s_0btag", varNameReco.Data()));
  hSR_Gr1_Clone[0][ivar]->Write(TString::Format("hWt_Gr1_%s_2btag_expYield", varNameReco.Data()));
  hCR_Gr1_Clone[0][ivar]->Write(TString::Format("hWt_Gr1_%s_0btag_expYield", varNameReco.Data()));

  hSR_Lo1[0][ivar]->Write(TString::Format("hWt_Lo1_%s_2btag", varNameReco.Data()));
  hCR_Lo1[0][ivar]->Write(TString::Format("hWt_Lo1_%s_0btag", varNameReco.Data()));
  hSR_Lo1_Clone[0][ivar]->Write(TString::Format("hWt_Lo1_%s_2btag_expYield", varNameReco.Data()));
  hCR_Lo1_Clone[0][ivar]->Write(TString::Format("hWt_Lo1_%s_0btag_expYield", varNameReco.Data()));
