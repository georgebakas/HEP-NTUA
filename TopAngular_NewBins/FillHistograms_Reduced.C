...
void FillHistograms_Reduced(TString y="2016", int sel = 0, int massWindow=1000)
{
  year =y;
 ...
 // cout<<"weight: "<<weights[f]<<endl;
  ...
  TFile *outFile;
  if(selection ==0)
    outFile = new TFile(TString::Format("%s/Histo_Data_%s_reduced_%d.root",year.Data(),year.Data(),massWindow), "RECREATE");
  else if(selection ==1)
    outFile = new TFile(TString::Format("%s/Histo_TT_Mtt-700toInf_reduced_%d.root",year.Data(),massWindow), "RECREATE");
  else if(selection ==2)
    outFile = new TFile(TString::Format("%s/Histo_QCD_HT300toInf_reduced_%d.root",year.Data(),massWindow), "RECREATE");
  else if(selection ==3)
    outFile = new TFile(TString::Format("%s/Histo_SubdominantBkgs_reduced_%d.root",year.Data(),massWindow), "RECREATE");
  else if(selection ==4)
    outFile = new TFile(TString::Format("%s/Histo_TT_NominalMC_reduced_%d.root",year.Data(), massWindow), "RECREATE");
