...
void graphTransferFactorsSpecific(TString year = "2016", TString variable = "jetPt0");

void graphTransferFactors_2TeV(TString year = "2016")
{
  const int NVAR = 4;
  TString vars[] = {"chi", "cosTheta_0", "cosTheta_1", "mJJ"};
  for(int ivar =0; ivar<sizeof(vars)/sizeof(vars[0]); ivar++)
  {
    graphTransferFactorsSpecific(year, vars[ivar]);
    //break;
  }
}


void graphTransferFactorsSpecific(TString year = "2016", TString variable = "chi")
{
  gStyle->SetOptStat(0);
  TH1F *hfData;
  TH1F *hfQCD;
  ...
  //my Signal region, is the reduced with mJJ > 2000 GeV
	infDataReduced = TFile::Open(TString::Format("%s/Histo_Data_%s_2000.root", str.Data(),str.Data()));

	infQCD = TFile::Open(TString::Format("%s/Histo_QCD_HT300toInf_1000.root", str.Data())); //this is now my SR
	infQCDReduced = TFile::Open(TString::Format("%s/Histo_QCD_HT300toInf_2000.root",str.Data())); //this is SR extension
