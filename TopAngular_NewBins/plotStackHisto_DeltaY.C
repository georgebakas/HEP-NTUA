...
void plotStackHisto_DeltaY(TString year, int mJJCut = 2000, int selMass= 2500, int selWidth=25)
{
  setTDRStyle();
  initFilesMapping();
  mass = selMass;
  ...
  //data file
  TFile *infData = TFile::Open(TString::Format("%s/HistoDeltaY_Data_%s_reduced_%d.root",year.Data(), year.Data(), mJJCut));
  ...
  //this is nominal
  TFile *infTT = TFile::Open(TString::Format("%s/HistoDeltaY_TT_NominalMC_reduced_%d.root",year.Data(), mJJCut));
  ...
  TFile *infQCD = TFile::Open(TString::Format("%s/HistoDeltaY_QCD_HT300toInf_reduced_%d.root",year.Data(), mJJCut));

  //subdominant file:
  TFile *infSub = TFile::Open(TString::Format("%s/HistoDeltaY_SubdominantBkgs_reduced_%d.root",year.Data(), mJJCut));
...
void plotStackHisto_Variable(TString year, TFile *infData, TFile *infTT, TFile *infQCD, TFile *infSub, TString variable, int mJJCut)
{
  ...
  hData = (TH1F*)infData->Get(TString::Format("hWt_Lo1_%s_2btag_expYield", variable.Data()));

  hTT = (TH1F*)infTT->Get(TString::Format("hWt_Lo1_%s_2btag_expYield", variable.Data()));
  //hTT = (TH1F*)infTT->Get(TString::Format("hSignal_%s", variable.Data())); //to use if SignalExtraction
  //if use data, uncomment
  hQCD = (TH1F*)infQCD->Get(TString::Format("hWt_Lo1_%s_2btag_expYield", variable.Data()));

  hSub = (TH1F*)infSub->Get(TString::Format("hWt_Lo1_%s_2btag_expYield", variable.Data()));
  ...
  else if (year.EqualTo("2017")) infZprime = TFile::Open(TString::Format("../Zprime/%s/HistoMassWindows_DY_ZprimeToTT_M%d_W%d_TuneCP2_PSweights_13TeV-madgraph-pythiaMLM-pythia8_19UL.root", year.Data(), mass, width));  
  ...
  TH1F *hZ = (TH1F*)infZprime->Get(TString::Format("hReco_Lo1_%s_%d", variable.Data(), mJJCut));
  ...
  can->Print(TString::Format("%s/StackPlots_DeltaY/DatavsMC_%s_%d_M%dW%d.pdf",year.Data(), variable.Data(), mJJCut, mass, width),"pdf");
