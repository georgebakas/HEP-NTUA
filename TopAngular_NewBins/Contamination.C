...
void Contamination(TString year)
{
  ...
  //qcd template
  TFile *infQCDMC = TFile::Open(TString::Format("%s/Histo_QCD_HT300toInf_1000.root", year.Data()));
  //ttbar
  TFile *infTTMC = TFile::Open(TString::Format("%s/Histo_TT_NominalMC_1000.root",year.Data()));  //nominal
  //TFile *infTTMC = TFile::Open(TString::Format("%s/Histo_TT_Mtt-700toInf_100.root",year.Data())); //mtt
  //subdominant bkgs
  TFile *infSubBkg = TFile::Open(TString::Format("%s/Histo_SubdominantBkgs_1000.root",year.Data()));  //nominal


  //get the histograms for all the variables:
  const int NVAR = 4;
  TString recoVarAll[NVAR] = {"chi", "cosTheta_0", "cosTheta_1", "mJJ"};
