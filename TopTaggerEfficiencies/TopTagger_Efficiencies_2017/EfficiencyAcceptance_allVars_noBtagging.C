...
void EfficiencyAcceptance_allVars_noBtagging(float selMvaCut=-0.1, bool saveTtagger= true)
{
  initGlobals();
  const int NVAR =7;
  const int N_MJJ = 10;
  ...
  for(int f=0; f<listOfFiles.size(); f++)
  {
	...
	if(isMatched > 1)
    {
	  ...
	  //qcout<<"----------"<<endl;
	      ...
	      if(partonCuts && recoCuts && tTaggerCut)
		  {
			  	for(int ivar = 0; ivar < NVAR-2; ivar++)
	  			{
...
/*
  ...
  */
  ...
  TFile *outFile;
  outFile = TFile::Open(TString::Format("Efficiencies_allVars_tTagger_%0.2f_deepCSV_noBtagging.root", selMvaCut), "UPDATE");
