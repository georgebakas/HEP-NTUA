...
void FillHistograms_Reduced_SystematicsFiles(TString file_name, TString ttbar_process, TString sysVariation = "mTop", TString year = "2016", float mJJCut = 1000 )
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  float norm = ((TH1F*)file->Get("eventCounter/GenEventWeight"))->GetSumOfWeights();
    ...
    cout<< TString::Format("HistoReduced_%d_%s_%s.root", (int)mJJCut, ttbar_process.Data(), sysVariation.Data())<< endl;
    TFile *outFile = TFile::Open(TString::Format("HistoReduced_%d_%s_%s.root", (int)mJJCut, ttbar_process.Data(), sysVariation.Data()), "RECREATE");
    outFile->cd();
    //write them to file
    for(int ivar = 0; ivar<NVAR; ivar++)
    {
