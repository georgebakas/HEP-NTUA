...
void ResponseMatrices_SystematicsFiles(TString file_name, TString ttbar_process, TString sysVariation = "mTop", TString year = "2016", float mJJCut=1000)
{
  globalYear = year;
  initFilesMapping();
  cout<<"YEAR: "<<year<<endl;
  ...
  } // end of NVAR purity stability

  TFile *outFile = TFile::Open(TString::Format("ResponsesEfficiency_%s_%s.root", ttbar_process.Data(), sysVariation.Data()), "RECREATE");
  //outFile->cd();
  //write them to file
  for(int ivar = 0; ivar<NVAR; ivar++)
  {
