...
void CombineUnfoldedMeasurements(TFile *outFile, TString isParton = "Parton")
{
  ...
  for (unsigned int var = 0; var < AnalysisConstants::unfoldingVariables.size(); var++)
  {
    ...
    for (unsigned int y = 0; y < AnalysisConstants::years.size(); y++)
    {
      ...
      {
        std::vector<TString> variationFiles = listFiles(TString::Format("%s/%s/Unfold_%s/OutputFile%s", baseInputDir.Data(), year.Data(), variation_dirs[i].Data()), isParton.Data());
