...
def executFitDiagnostics(masses, mass_cut):

    for mass in masses:
        file_name = "datacard_chi_SR_"+mass+"_cut_"+mass_cut+".txt"
        fit_command = 'combine -M FitDiagnostics -d '+file_name+' -n _fit_result.'+mass+"Cut"+mass_cut+' --saveShapes --saveWithUncertainties'
        print ""
        print ">>> " + fit_command
...
#    legend.AddEntry(green, "Asymptotic CL_{s} #pm 2 std. deviation",'f')
    ...
    print 'UpperLimit_w{}_mJJ{}.png'.format(width, mass_cut)
    label = 'UpperLimit_w{}_mJJ{}.png'.format(width, mass_cut)
    c.SaveAs(label)
    c.Close()
...
def main(width, mass_cut):
    ...
    executFitDiagnostics(file_names, mass_cut)
