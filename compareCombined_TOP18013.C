...
void compareCombined_TOP18013()
{

  std::vector <TString> variables = {"jetPt0", "jetPt1", "mJJ", "yJJ", "ptJJ", "jetY0", "jetY1"}; 
  
  TFile *inf_kousouris;
  TFile *inf_combined = TFile::Open(TString::Format("2.4.0/outFile%s.root", "Parton"));
  for(int ivar=0; ivar<variables.size(); ivar++)
  {
    TString variable = variables[ivar];

    // Get our combined result 
    TH1F *h_comb = (TH1F*)inf_combined->Get(TString::Format("combined_%s", variable.Data()));

    inf_kousouris = TFile::Open(TString::Format("hepData/Results-TOP18013/CrossSection_%s_%s.root", "Parton", variable.Data()));
    TH1F *h_TOP = (TH1F*)inf_kousouris->Get("CrossSection_Unfolded_Nominal");
    //TH1F *h_TOP = (TH1F*)inf_kousouris->Get("Yield_Unfolded_Nominal");

    h_comb->SetMarkerStyle(21);
    h_TOP->SetMarkerStyle(20);
    h_comb->SetMarkerColor(kBlack);
    h_TOP->SetMarkerColor(kRed);
    h_comb->SetLineColor(kBlack);
    h_TOP->SetLineColor(kRed);

    
    TLegend *leg = new TLegend(0.5,0.6,0.7,0.8);
    leg->AddEntry(h_comb, "Combined Result", "lp");
    leg->AddEntry(h_TOP, "TOP-18-013", "lp");
    ...
    can->Print(TString::Format("VariationHandling/ComparisonWithTOP18013/comparison_%s.pdf", variable.Data()), "pdf");
  }
}
/*
    //----------------------------------FOR 2018---------------------------------------------------------------------
