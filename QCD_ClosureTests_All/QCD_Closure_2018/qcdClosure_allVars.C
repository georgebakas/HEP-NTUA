...
void qcdClosure_allVars(bool isSig = false)
{
  ...
  */
  ...
  if(isSignal) 
  {
	  if(isDeepCSV) outFile = new TFile(TString::Format("SignalOutput_AllRegions_%0.2f_deepCSV_pT600_.root", selMvaCut), "UPDATE");
	  else outFile = new TFile(TString::Format("SignalOutput_AllRegions_%0.2f_CSVv2.root", selMvaCut), "UPDATE");
  }
  else 
  {
	  if(isDeepCSV) outFile = new TFile(TString::Format("BkgOutput_AllRegions_%0.2f_deepCSV_pT600_.root",selMvaCut), "UPDATE");
