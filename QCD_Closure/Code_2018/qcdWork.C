...
void initFileNames()
{
  listOfFiles.push_back("/eos/cms/store/user/gbakas/ttbar/topTagger/mc-2018/Signal/TT_Mtt-1000toInf_TuneCP5_13TeV-powheg-pythia8.root");
...
void qcdWork(float selMvaCut = 0.1, bool isDeepCSV = true)
{
  initGlobals();
  ...
  if(isDeepCSV)
  {
	 infQCD= TFile::Open(TString::Format("Closure_QCDBkg_Chi_%0.1f_deepCSV.root", selMvaCut));
     infTT = TFile::Open(TString::Format("Output_TT_QCD_Reco_Chi_%0.1f_deepCSV.root", selMvaCut));
  }
  else
  {
	 infQCD= TFile::Open(TString::Format("Closure_QCDBkg_Chi_%0.1f.root", selMvaCut));
     infTT = TFile::Open(TString::Format("Output_TT_QCD_Reco_Chi_%0.1f.root", selMvaCut));  
  }
  ...
  TH1F *hChi_QCD_1btag = (TH1F*)infQCD->Get("hChi_QCD_1btag");
  TH1F *hCos_QCD_1btag = (TH1F*)infQCD->Get("hCos_QCD_1btag");
